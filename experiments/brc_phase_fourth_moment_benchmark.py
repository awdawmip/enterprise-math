"""Benchmark the fourth-order lifted-moment BRC phase tail.
...
2. the fourth-order tracker performs twenty total seed roots and then exact
...
def _population(bits: int, samples: int) -> tuple[tuple[int, int], ...]:
    ...
    for _ in range(samples):
        ...
        values.append((n, aligned_fourth_phase_tail_threshold(n)))
    return tuple(values)
...
def _phase_pipeline(
    ...
    tracker = FourthPhaseMomentTracker(n, start)
    survivors = exact_squares = 0
...
def _phase_stats(
    ...
    for n, start in values:
        ...
        for _ in range(steps):
            ...
            if tracker.last_mode == "FOURTH_PHASE_RECURRENCE":
...
def benchmark_rows() -> list[dict[str, object]]:
    ...
    for bits, samples, steps in CONFIGS:
        ...
        if _phase_pipeline(values[0][0], values[0][1], steps) != reference:
            raise AssertionError("fourth-phase pipeline changed observed semantics")
        ...
        rows.append(
            {
                ...
                "fourth_phase_moment_seconds": phase_seconds,
                "direct_over_fourth_phase_speedup": direct_seconds / phase_seconds,
            }
        )
    return rows
...
if __name__ == "__main__":
    target = (
        ...
        / "research_artifacts"
        / "brc_phase_fourth_moment_benchmark_20260907.csv"
    )
    write_csv(target)
    print(target)
