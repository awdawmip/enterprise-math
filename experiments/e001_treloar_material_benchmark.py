"""Real-material loading benchmark against classic Treloar rubber data.

The Enterprise Math candidate is fitted with integer arithmetic. Neo-Hooke,
Mooney-Rivlin and Yeoh calculations are external continuous least-squares
baselines only; they are not imported into the project core.
"""
...
def _rmse(targets: tuple[float, ...], predictions: tuple[float, ...]) -> float:
    return sqrt(
        sum((t - p) ** 2 for t, p in zip(targets, predictions, strict=True))
        / len(targets)
    )
