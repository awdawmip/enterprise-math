"""E001 real-material validation benchmark with deterministic held-out folds.

This experiment complements the all-points Treloar fit.  It reports a fixed
five-fold interleaved validation score and compares it with standard continuous
hyperelastic baselines fitted on the same training folds.  Cross-validation and
all continuous models are established engineering/statistical tools.
"""
...
def _continuous_five_fold_rmse(columns: tuple[tuple[float, ...], ...]) -> float:
    ...
    for fold in range(5):
        ...
        train_columns = tuple(tuple(column[index] for index in train) for column in columns)
        train_targets = tuple(targets[index] for index in train)
        coefficients = _least_squares(train_columns, train_targets)
        for index in test:
            prediction = sum(coefficients[j] * columns[j][index] for j in range(len(columns)))
...
def print_integer_validation(amplitude: int) -> None:
    ...
    print(
        f"enterprise_integer A={amplitude} full_shape=(G{full.input_root_power},H{full.output_hardening_power},{full.output_scale}) "
