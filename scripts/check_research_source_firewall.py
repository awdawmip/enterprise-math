...
The audit installs the canonical fault-isolated task view first. An unrelated,
exactly quarantined publication fault must not disable source-firewall checking
for every healthy task; new or drifted quarantine faults still fail closed in
...
def main() -> int:
    try:
        ...
        print(f"source firewall audit: FAIL: cannot resolve canonical current publications: {exc}")
        return 1
