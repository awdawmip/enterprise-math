...
def main():
    ...
    # 1. Exact rank-one torsion-free no-go: Aut(Z) = {+1,-1}.
    free_rank1_orbits = {}
    for eps in (-1, 1):
        seq = []
        x = 1
    ...
    # 2. Exhaust the rank-one split torsion normal form at the first
    # two possible kernel sizes. Complexity order makes m<=3 complete
    # for proving the least non-sign orbit: m=2 is the only smaller
    # nontrivial cyclic kernel; m=3 is the first possible orbit size >2.
    torsion_search = {}
    raw_m3_survivors = []
    for m in (2, 3):
        rows = []
    ...
    # 3. Exact annihilator/order data of the least carrier.
    gens = [(1, 0), (0, 1)]
    rel_checks = {
    ...
    # 4. Sign layer remains central and exact dark cancellation survives.
    sign_checks = {
    ...
    # 5. Unique reversal fixing the old elementary occurrence.
    reversal_checks = {
    ...
    # 6. Composition to depth 4: exponent/power action is a functor.
    composition_cases = 0
    ...
    # Exhaust all edge exponent assignments around one commuting diamond.
    diamond_cases = 0
    ...
    # 7. Weak three-sector covariance is exact copy, not globalization.
    sectors = ["12", "23", "31"]
    ...
    # 8. Multiplication classification on the least additive carrier.
    product_survivors = []
    ...
    # 9. Counterfactual torsion-free rank-2 primitive-cyclic companions.
    companions = {
        "trace_-1": ((0, -1), (1, -1)),
        "trace_0": ((0, -1), (1, 0)),
        "trace_1": ((0, -1), (1, 1)),
    ...
    # 10. Mandatory ablation countermodels.
    U = ((1, 1), (0, 1))
