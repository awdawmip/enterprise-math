#!/usr/bin/env python3
"""Exact standard-library verifier for the frozen Q28 n=12 certificate."""
...
RESULT="RR-25000691E640F031C27A"
EXECUTION="ER-B1DE2191BD4CF4AEF191"
RID="EM-P000-89C4F2"
...
def packet(a):
    ...
    leg0=tuple(sorted(set(p))); ids={x:i for i,x in enumerate(leg0)}
    col=[ids[x] for x in p]
    ...
    for t in range(n):
        ...
        leg=tuple(sorted(set(sig))); ids={x:i for i,x in enumerate(leg)}
        new=[ids[x] for x in sig]
...
def aut(a,p,col):
    ...
    keys=[(deg[v],p[v],col[v]) for v in range(n)]
    buckets={}
...
def load(path):
    ...
    for s in m['payload_shards']:
        ...
        ck(h==obj['raw_sha256']==s['raw_sha256'],'shard raw hash')
...
def verify(path):
    ...
    for r in (2,4,6,8,10,12):
        ck(connected(12-r,r)==COUNTS[r],f'count r={r}')
        meta=m['sectors'][str(r)]; dat=payload[str(r)]
        rows=dat['graph6']; autos=dat['aut_sizes']
        ck(len(rows)==len(autos)==REPS[r]==meta['representative_count'],'rep count')
        ck(rows==sorted(rows),'sort')
        ...
        for s,fa in zip(rows,autos):
            ...
            ck(Counter(x.bit_count() for x in a)==Counter({3:r,2:12-r}),'degree')
            p,c,e=packet(a)
            aa=aut(a,p,c);ck(aa==fa and fac%aa==0,'aut')
            orbit+=fac//aa;enc.append(e);allenc.append(e)
        ck(len(enc)==len(set(enc)),'packet collision')
        enc.sort()
        dig=hashlib.sha256(('\n'.join(enc)+'\n').encode()).hexdigest()
        ck(dig==meta['packet_image_sha256'],'sector digest')
        ck(orbit==COUNTS[r]==meta['normalized_label_orbit_sum']==meta['expected_normalized_connected'],'orbit cover')
        nr+=len(rows);lab+=orbit
    ck(nr==NREP and lab==TOTAL and len(allenc)==len(set(allenc))==NREP,'global cover/collision')
    allenc.sort()
    dig=hashlib.sha256(('\n'.join(allenc)+'\n').encode()).hexdigest()
    ck(dig==m['combined_packet_image_sha256'],'combined digest')
    return nr,lab,len(allenc),dig

def main():
    d=Path(__file__).resolve().parents[1]/'research_artifacts'/'P000_PHILOSOPHY_FIRST_RETURN_PROFILE_1WL_N12_COLLISION_FRONTIER'/'P000_Q28_RETURN_PROFILE_1WL_N12_EXACT_ORBIT_CERTIFICATE_V1.json'
