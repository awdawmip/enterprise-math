...
def vp(n,p):
    if n == 0:
        raise ValueError("finite p-adic checker domain excludes zero")
    n=abs(n); c=0
    while n%p==0:
        n//=p; c+=1
    return c
...
for x in product(vals3, repeat=6):
    ...
    qv=Q(x)
    if d>0:
        check(qv!=0, "unique minimum forbids Q=0")
        check(vp(qv,3)==min(alpha), "unique-min valuation law")
    if qv==0:
        check(d==0, "Q=0 forces tropical tie")
