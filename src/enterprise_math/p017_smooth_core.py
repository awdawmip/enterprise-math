...
natural multiplicity-preserving refinement of the squarefree transverse-support
language used elsewhere in P017.
"""
...
def square_basin_smooth_core(k: int, n: int) -> dict[str, object]:
    ...
    The returned ``smooth_core`` is

        S_k(n) = product_{p<=k} p^{v_p(n)}.

    The returned ``large_tail`` is ``n // S_k(n)`` and is asserted to be either
    one or a prime strictly greater than k.
    """
