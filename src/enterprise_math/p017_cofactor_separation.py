"""Canonical P017 L053 cofactor-window separation.
...
def cofactor_window_pair_separation(k: int, left_prime: int, right_prime: int) -> dict[str, int]:
    """Return L053 separation data for primes p<r<=k and k>=4."""
    _require_positive("k", k)
    if k < 4:
        raise ValueError("L053 separation requires k >= 4")
    ...
    if gap < 0:
        raise AssertionError("L053 cofactor windows overlap")
...
def least_factor_strip_injection(k: int) -> dict[str, object]:
    ...
    _require_positive("k", k)
    if k < 4:
        raise ValueError("L053 injection requires k >= 4")
