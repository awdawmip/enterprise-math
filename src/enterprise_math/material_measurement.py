...
are intentionally opaque here: dimensional algebra is outside the E001 scope.
"""
...
class ExactMeasurement:
    """One reduced exact rational engineering value with an opaque unit tag."""
...
class FiniteMeasurementAxis:
    """One finite observation axis expressed as integer counts per unit."""
    ...
    def exact_value(self, count: int) -> ExactMeasurement:
        """Return the reduced exact rational represented by one observation."""
...
class FiniteMaterialDataset:
    """Paired finite material observations with explicit measurement scales."""
...
def model_precision_is_independent(dataset: FiniteMaterialDataset, amplitude: int) -> bool:
    """Validate and expose that model precision is not dataset measurement scale.

    This deliberately returns a Boolean rather than transforming either object:
    the benchmark layer must choose ``A`` independently of the experimental
    ``scale_factor`` values.  Equality between them is permitted but has no
    special semantics.
    """
