"""Exact first-moment mirror-incidence formulas for P017.

L050 evaluates the surviving-radius count and every transverse-prime mirror
incidence using only square-free inclusion-exclusion, modular inverses, and floor
counts of residue classes. L051 turns the already-proved L045 necessary condition
into a finite sufficient prime-existence certificate.
"""
...
def surviving_radius_count_formula(k: int) -> int:
    """L050 formula for |S_k| by Mobius inclusion-exclusion over anchor primes."""
    _require_k(k)
    limit = k - 1
    return sum(
        mu * (limit // a)
...
def transverse_prime_incidence_formula(k: int, prime: int) -> int:
    """L050 formula for N_p(k), the surviving mirror incidences of one transverse p."""
    _require_k(k)
...
def mirror_incidence_formula(k: int) -> dict[str, object]:
    """Evaluate L050 and the L051 sufficient prime-existence certificate."""
    _require_k(k)
    center = k * (k + 1)
