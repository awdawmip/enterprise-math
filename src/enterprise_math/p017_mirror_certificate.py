"""Exact first-moment formulas for the P017 mirror-certificate annex.

MC01 evaluates the surviving-radius count and every transverse-prime mirror
incidence using square-free inclusion-exclusion, modular inverses, and floor
counts of residue classes. The resulting J_k feeds the aggregate certificate.
"""
...
def surviving_radius_count_formula(k: int) -> int:
    """MC01 formula for |S_k| by Mobius inclusion-exclusion over anchor primes."""
    _require_k(k)
    limit = k - 1
    return sum(
        mu * (limit // a)
...
def transverse_prime_incidence_formula(k: int, prime: int) -> int:
    """MC01 formula for N_p(k), the surviving mirror incidences of one transverse p."""
    _require_k(k)
...
def mirror_incidence_formula(k: int) -> dict[str, object]:
    """Evaluate MC01 first-moment data."""
    _require_k(k)
    center = k * (k + 1)
