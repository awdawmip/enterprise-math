...
FIRST_SUPPORT_COLLISION_PRIME = 369_581
FIRST_SUPPORT_COLLISION_MIDPOINT = 184_790
COLLISION_ZERO_INDEX = 8
EXPECTED_MIDPOINT_LIFT_QUOTIENT = 153_310
EXPECTED_DEFECT_VALUATION = -1


def first_counterexample_basic_arithmetic() -> bool:
    prime = FIRST_SUPPORT_COLLISION_PRIME
    midpoint, _ = composite_boundary_half_witness(prime)
    if midpoint != FIRST_SUPPORT_COLLISION_MIDPOINT:
...
def first_counterexample_relation_exponent() -> int:
    midpoint = FIRST_SUPPORT_COLLISION_MIDPOINT
...
def first_counterexample_low_support_zero_hits() -> tuple[int, ...]:
...
def first_counterexample_midpoint_is_simple() -> bool:
    quotient = half_index_lift_quotient(FIRST_SUPPORT_COLLISION_PRIME)
...
def first_counterexample_flux_correction() -> int:
    correction = half_defect_flux_correction(FIRST_SUPPORT_COLLISION_PRIME)
    if correction != -2:
        raise AssertionError("transfer correction must be -2")
    return correction


def first_counterexample_defect_valuation() -> int:
    value = franel_defect_valuation(
        FIRST_SUPPORT_COLLISION_MIDPOINT,
