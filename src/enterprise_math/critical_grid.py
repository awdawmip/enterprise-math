...
This module is the executable specification for P018 Supplement 13.  The
subtraction-free layer keeps endpoint pairs and composes adjacent pairs by their
shared endpoint.  When states are natural numbers, the same rectangle admits an
exact signed-difference coordinate whose two decompositions agree by finite
response/telescoping identities.
...
def rectangle_endpoint_pairs(
    ...
    With ``a=G0(F0(x))``, ``b=G0(F1(x))``, ``c=G1(F0(x))``,
    ``d=G1(F1(x))``, the two decompositions are

    ``(a,b) ; (b,d)`` and ``(a,c) ; (c,d)``.

    Both must reduce to the same outer pair ``(a,d)``.
    """
    f0 = first_left(state)
...
def rectangle_holonomy_decompositions(
    ...
    If ``h = F1(x)-F0(x)`` and ``delta_G(y)=G1(y)-G0(y)``, then

    ``outer = R_G0(F0(x),h) + delta_G(F1(x))``

    and

    ``outer = delta_G(F0(x)) + R_G1(F0(x),h)``.
    """
    _require_natural_output("state", state)
    f0 = _require_natural_output("F0 output", first_left(state))
    f1 = _require_natural_output("F1 output", first_right(state))
...
def rectangle_variation_identity(
    ...
    ``delta_G(F1(x)) - delta_G(F0(x))`` equals
    ``R_G1(F0(x),h) - R_G0(F0(x),h)``.
    """
    _require_natural_output("state", state)
    f0 = _require_natural_output("F0 output", first_left(state))
    f1 = _require_natural_output("F1 output", first_right(state))
...
def cancellation_example(state: int = 1) -> dict[str, int]:
    """Return a concrete rectangle with zero outer holonomy but nonzero local edges.

    Take ``F0(x)=x``, ``F1(x)=x+1``, ``G0(y)=y`` and
    ``G1(y)=max(y-1,0)``.  At every ``state>=1`` the outer endpoints coincide,
    while both the horizontal displacement and the pointwise G-family defect are
    nonzero.  This is a regression against the false inference
    ``outer zero => every local defect zero``.
    """
    _require_natural_output("state", state)
    if state < 1:
