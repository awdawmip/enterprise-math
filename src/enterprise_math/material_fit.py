...
def project_interval_nearest(value: int, lower: int, upper: int, amplitude: int) -> int:
    ...
    Ties use the non-negative half-up convention.  The routine is intended for
    monotone engineering loading coordinates and therefore requires the input
    observation to lie in the declared finite interval.
    """
...
def root_basin_versine_basis(
    ...
    """Build a monotone finite loading basis from integer deformation samples."""
    _require_positive("amplitude", amplitude)
...
def predict_scaled_basis(
    ...
    """Map a ``0..A`` basis to one non-negative integer engineering output scale."""
    _require_positive("amplitude", amplitude)
...
def squared_error(targets: tuple[int, ...] | list[int], predictions: tuple[int, ...] | list[int]) -> int:
    """Return exact integer sum of squared errors."""
...
def absolute_error(targets: tuple[int, ...] | list[int], predictions: tuple[int, ...] | list[int]) -> int:
    """Return exact integer L1 error."""
    if len(targets) != len(predictions):
        raise ValueError("targets and predictions must have equal length")
    total = 0
    for target, prediction in zip(targets, predictions, strict=True):
...
class IntegerMaterialFit:
    """One finite constitutive-curve fit with exact integer diagnostics."""
...
def fit_integer_output_scale(
    ...
    """Exhaustively fit the one integer output scale on a bounded domain.

    The bounded search is intentional: engineering calibration must declare the
    physically admissible output scale instead of relying on an unbounded real
    optimizer.  Ties are resolved toward the smaller output scale.
    """
    _require_positive("amplitude", amplitude)
    ...
    for output_scale in range(max_output_scale + 1):
        ...
        max_error = max(abs(t - p) for t, p in zip(target_values, predictions, strict=True))
