"""Primitive A3 relation structure plus explicit integer scale.

Semantic replay from the historical relation-quotient branch. A weighted
relation state factors by ``g=gcd(capacities)`` as

    m = g*m_hat,
    Z = g*Z_hat,

with primitive capacities ``gcd(m_hat)=1``. Partition coarsening may introduce
an exact integer scale carry ``h=gcd(A*m_hat)``; the new scale is ``g*h``.
"""
