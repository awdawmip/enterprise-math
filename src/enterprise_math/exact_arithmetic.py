...
Division is allowed to remain an unevaluated structural state.  Whenever a
quotient-derived value is materialized, evaluation must pass through the BRC
facade below and return an explicit quotient/remainder/collapse trace.
...
class BRCScaledReadout:
    """Exact finite-scale readout before any textual decimal formatting."""
...
class DecimalReadout:
    """UI/readout representation derived from an integer-scale BRC evaluation."""
...
def brc_decimal_readout(expr: DivisionExpr, digits: int) -> DecimalReadout:
    ...
    Formatting uses decimal text slicing rather than another arithmetic division.
    The residual remains explicit and the returned text is therefore only a
    readout, never the native exact state.
    """
    scale = decimal_scale(digits)
    scaled = brc_scaled_evaluate(expr, scale)
    raw = str(scaled.scaled_value)
    if digits == 0:
        text = raw
    else:
