...
and shell counts are generated by repeated one-slot composition, not by an a
priori continuous metric space.
...
def admissible_one_slot_values(maximum_grade: int, grade: Grade) -> tuple[int, ...]:
    ...
    # Any admissible integer x must satisfy g(x)<=T.  For project grades used
    # here g(x)>=|x| for |x|>=1, so [-T,T] is a safe finite search envelope.
    values = []
