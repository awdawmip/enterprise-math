"""Cross-side mirror incidence and aggregate certificates for the P017 mirror annex.

MC02 evaluates ordered lower/upper transverse-prime incidences by exact CRT.
MC03 factors prime-free behavior into U=J-2|S| and V=E-J+|S|. MC05 adds
the aggregate quadratic bound 4V<=U^2. MC06 certifies a basin prime whenever
any of these necessary prime-free constraints is violated.
"""
...
def ordered_cross_incidence_formula(k: int, lower_prime: int, upper_prime: int) -> int:
    """MC02 formula for N_{p->q}(k), with p on M-r and q on M+r."""
    _require_k(k)
    ...
    if lower_prime == upper_prime:
        raise ValueError("mirror separation forbids the same transverse prime on both sides")
...
def cross_side_incidence_formula(k: int) -> dict[str, object]:
    """Evaluate MC02 and all MC03-MC06 aggregate slacks/certificates."""
    _require_k(k)
...
def aggregate_mirror_certificate(k: int) -> dict[str, object]:
    """Compact MC06 three-channel certificate output."""
