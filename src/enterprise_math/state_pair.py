...
Semantically replayed from historical P018 research. Pair and kernel
constructions are elementary established mathematics; the purpose here is to
retain their precision-specific role as a weaker substrate beneath numeric
defect coordinates and deterministic irreversibility.
"""
...
def diagonal_is_absorbing(operation: Callable[[X], Y], state: X) -> bool:
    """Check diagonal absorption at one state."""
...
def kernel_equals_diagonal_preimage(
    ...
    """Check that the kernel relation is the diagonal preimage under pair-map."""
...
def pair_to_difference(first: int, second: int) -> tuple[int, int]:
    """Coordinate map ``(a,b) -> (a,b-a)`` on natural states."""
...
def endpoint_pair(
    ...
    """Endpoint pair for two parallel deterministic paths."""
...
def critical_square_pair(
    ...
    """Endpoint pair ``(Q(F_e(x)), F_d(Q(x)))`` for a precision square."""
...
def kernel_monotonic_under_suffix(
    ...
    """Check that kernel membership persists under a deterministic suffix."""
