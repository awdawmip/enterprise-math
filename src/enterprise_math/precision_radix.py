...
executable pressure test for P018 Supplements 07--09; mixed-radix arithmetic and
its coordinate changes are established mathematics, not a novelty claim.
"""
...
def staged_carry(outer_radix: int, inner_radix: int, first: int, second: int) -> int:
    """Carry to the coarse endpoint after a two-level mixed-radix addition.

    ``first`` and ``second`` are total details in ``[0, outer_radix*inner_radix)``.
    The inner carry is transported into the outer digit before the final carry is
    decided.  The result should equal direct carry at the product radix.
    """
    _require_positive("outer_radix", outer_radix)
    _require_positive("inner_radix", inner_radix)
    total = outer_radix * inner_radix
