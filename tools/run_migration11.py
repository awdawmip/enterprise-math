#!/usr/bin/env python3
"""Reproduce the M11 readout source slice; no installation or remote execution."""
...
def main():
    ...
    parser.add_argument('--out',type=Path,default=ROOT/'evidence/migration11/tests.json')
    ...
    names=('test_multiplication_lab','test_angular_dispersion','test_certified_hex',
           ...
           'test_multiplicative_readouts')
    modules=[load(ROOT/'tools/nollm_visual_toolkit/tests'/f'{name}.py','m11_'+name) for name in names]
    ...
    record={'schema':'M11_READOUT_VALIDATION_V1','tests':result.testsRun,
            'new_readout_tests':counts['test_multiplicative_readouts'],
            'failures':len(result.failures),'errors':len(result.errors),'skips':len(result.skipped),
            'groups':counts,'finite_cases':modules[-1].COUNTS,'policy_exit':policy.returncode,
            'scope':'Pinned readout source slice and retained arithmetic regressions; not full M10 startup/DOM, full package, native navigation or independent review',
            'legacy_expectations_advanced':4,
