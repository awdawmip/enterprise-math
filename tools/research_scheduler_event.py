...
stdout; callers append that exact object as one Issue #240 comment.
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap = sp.add_parser("approve"); ap.add_argument("--task-id", required=True); ap.add_argument("--reviewer-id", required=True); ap.add_argument("--review-claim-id", required=True); ap.add_argument("--taskbook-ref", required=True); ap.add_argument("--review-ref", required=True); ap.add_argument("--at", required=True)
    ...
    if args.cmd == "review-claim":
        ...
        emit(event); return 0
    if args.cmd == "approve":
        event=base(args,"APPROVE"); event.update(reviewer_id=args.reviewer_id,review_claim_id=args.review_claim_id,taskbook_ref=args.taskbook_ref,review_ref=args.review_ref); emit(event); return 0
    ...
    if args.cmd == "review":
        event=base(args,"REVIEW"); event.update(reviewer_id=args.reviewer_id,review_claim_id=args.review_claim_id,verdict=args.verdict,review_ref=args.review_ref,findings=args.finding)
