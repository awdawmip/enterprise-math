#!/usr/bin/env python3
"""Enterprise Math research scheduler V2 implementation.

The V2 control plane has four non-negotiable properties:

* every official task is registry-visible (unknown taskbooks are imported as
  non-dispatchable ORPHANED migration records rather than silently ignored);
* PUBLISH never implies READY; Driver DISPATCH review is a separate transition;
* RETURN never implies DONE; Driver RETURN review is a separate transition;
* V2 lease expiry is a durable ORPHANED event/state, while historical V1 lease
  expiry is replay-compatible and still leaves durable orphan provenance.

The implementation is intentionally network-free. GitHub Issue #240 is the
append-only runtime log; callers export its JSON events and feed them to this
pure reducer.
"""
...
PATCHABLE_TASK_FIELDS = {
    "owner", "priority", "leverage", "frontier", "next_action",
    "source_refs", "evidence_status", "tags",
}
...
def load_scheduler_bundle(path: pathlib.Path = DEFAULT_CONFIG) -> dict[str, Any]:
    ...
    if isinstance(seed_name, str) and seed_name.strip():
        seed = load_json(path.parent / seed_name)
        ...
        for item in seed.get("tasks", []):
            ...
            tasks.append(cloned)
        _apply_seed_overrides(tasks, materialized.get("seed_task_overrides", {}))
...
def validate_scheduler(config: dict[str, Any], owners: dict[str, Any]) -> list[str]:
    ...
    leverages = set(policy.get("leverage_order", []))
    ...
    for index, task in enumerate(tasks):
        ...
        if leverages and task.get("leverage") not in leverages:
            errors.append(f"{task_id}: invalid leverage {task.get('leverage')!r}")

        owner = task.get("owner")
        kind = task.get("kind")
        if kind == "RESEARCH":
            if state in {"PUBLISHED", "ORPHANED"} and owner == "taskbook/unassigned":
                pass
        ...
        block = task.get("hard_block")
        if block is not None and not complete_hard_block(block):
            errors.append(f"{task_id}: partial hard_block is invalid")
        if state == "BLOCKED" and not complete_hard_block(block):
...
def load_events(path: pathlib.Path | None) -> list[dict[str, Any]]:
    ...
    if not all(isinstance(event, dict) for event in events):
        raise SchedulerError("every event must be a JSON object")
    return events
...
def _publication_task(config: dict[str, Any], event: dict[str, Any]) -> tuple[dict[str, Any] | None, str | None]:
    ...
    contract = config.get("publish_contract", {})
    if role not in set(contract.get("allowed_publisher_roles", [])):
        return None, f"PUBLISH invalid publisher_role {role!r}"
    publisher_id = event.get("publisher_id")
    if role in set(contract.get("publisher_id_required_for_roles", [])) and not valid_publisher_id(role, publisher_id):
    ...
    for field in contract.get("required_task_fields", []):
        value = payload.get(field)
        if not isinstance(value, str) or not value.strip():
            return None, f"PUBLISH task missing {field}"
    policy = config.get("selection_policy", {})
    if payload.get("priority") not in set(policy.get("priority_order", [])):
        return None, f"PUBLISH invalid priority {payload.get('priority')!r}"
    if payload.get("leverage") not in set(policy.get("leverage_order", [])):
    ...
    task.update({
        ...
        "publisher_role": role,
        "publisher_id": publisher_id.strip().upper() if isinstance(publisher_id, str) else None,
        "publisher_actor": event.get("actor"),
        "published_at": published_at.isoformat(),
        "publish_ref": event.get("publish_ref") or event.get("taskbook_ref"),
        "taskbook_ref": event.get("taskbook_ref") or task.get("taskbook_ref"),
        "taskbook_policy_state": event.get("taskbook_policy_state"),
        "registry_source": "RUNTIME_PUBLISH_EVENT",
        "publication_review_exempt": False,
    })
...
def _taskbook_orphan(path: pathlib.Path, meta: dict[str, Any], *, root: pathlib.Path) -> dict[str, Any]:
    task_id = str(meta["task_id"])
    kind = meta.get("kind") if meta.get("kind") in {"RESEARCH", "GOVERNANCE"} else ("GOVERNANCE" if task_id.startswith("GS-") else "RESEARCH")
    owner = meta.get("owner")
    if kind == "GOVERNANCE":
        owner = "governance"
    elif not isinstance(owner, str) or not owner.strip():
        owner = "taskbook/unassigned"
    review = meta.get("policy_review") if isinstance(meta.get("policy_review"), dict) else {}
    rel = str(path.relative_to(root))
    original_state = meta.get("base_state")
    return {
        "task_id": task_id,
        ...
        "owner": owner,
        "base_state": "ORPHANED",
        "priority": meta.get("priority") if meta.get("priority") in {"P0", "P1", "P2", "P3"} else "P2",
        "leverage": meta.get("leverage") if meta.get("leverage") in {"HIGH", "MEDIUM", "LOW"} else "MEDIUM",
        "frontier": meta.get("frontier") or f"Legacy/unregistered taskbook discovered during Scheduler V2 migration: {rel}",
        "next_action": meta.get("next_action") or "Driver must recover, close, reject, or supersede this discovered orphan after inspecting its current evidence.",
        "dependencies": meta.get("dependencies") if isinstance(meta.get("dependencies"), list) else [],
        "source_refs": list(meta.get("source_refs", [])) if isinstance(meta.get("source_refs"), list) else [],
        "evidence_status": meta.get("evidence_status") or "UNREGISTERED_TASKBOOK_DISCOVERED",
        "last_progress_ref": meta.get("last_progress_ref") or rel,
        "last_progress_at": meta.get("last_progress_at") if isinstance(meta.get("last_progress_at"), str) else "1970-01-01T00:00:00+00:00",
        "hard_block": None,
        "tags": list(meta.get("tags", [])) if isinstance(meta.get("tags"), list) else [],
        "identity_lane": meta.get("identity_lane") or task_id.split("-", 2)[1] if "-" in task_id else "LEGACY",
        "taskbook_ref": rel,
        "taskbook_policy_state": review.get("review_state"),
        "registry_source": "TASKBOOK_ORPHAN_DISCOVERY",
        "publication_review_exempt": False,
        "initial_orphan_record": {
            "orphaned_at": "2026-08-25T00:00:00+08:00",
            "reason": "UNREGISTERED_TASKBOOK_DISCOVERED_DURING_V2_MIGRATION",
            "claim_id": None,
            "actor": None,
            "researcher_id": None,
            "last_progress_ref": meta.get("last_progress_ref") or rel,
            "next_action": meta.get("next_action") or "Driver recovery audit required",
            "derived": True,
            "discovered_by": "SCHEDULER_V2_REGISTRY_DISCOVERY",
            "source_ref": rel,
            "original_taskbook_state": original_state,
        },
    }


def discover_taskbooks(config: dict[str, Any], known: set[str], *, root: pathlib.Path = ROOT) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    tasks: list[dict[str, Any]] = []
    issues: list[dict[str, Any]] = []
    policy = config.get("registry_integrity", {})
    if not policy.get("auto_register_untracked_taskbooks_as_orphaned", True):
        return tasks, issues
    ...
    for path in sorted(directory.glob("*.md")):
        ...
        try:
            ...
            issues.append({"task_id": None, "source_ref": str(path.relative_to(root)), "reason": f"malformed taskbook: {exc}"})
            continue
        task_id = meta.get("task_id")
        if not isinstance(task_id, str) or not task_id.strip():
            issues.append({"task_id": None, "source_ref": str(path.relative_to(root)), "reason": "frontmatter missing task_id"})
            continue
        if task_id in seen_local:
            issues.append({"task_id": task_id, "source_ref": str(path.relative_to(root)), "reason": "duplicate taskbook task_id"})
            continue
        seen_local.add(task_id)
        if task_id in known:
            continue
        task = _taskbook_orphan(path, meta, root=root)
        tasks.append(task)
        known.add(task_id)
    return tasks, issues


def collect_tasks(config: dict[str, Any], events: list[dict[str, Any]], *, root: pathlib.Path = ROOT) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for index, event in enumerate(events):
        ...
        if isinstance(task_id, str) and task_id in seen:
            issues.append({"index": index, "task_id": task_id, "reason": "duplicate PUBLISH task_id"})
            continue
        task, error = _publication_task(config, event)
        if error:
            issues.append({"index": index, "task_id": task_id, "reason": error})
            continue
        assert task is not None
        tasks.append(task)
        seen.add(task["task_id"])

    discovered, discovery_issues = discover_taskbooks(config, seen, root=root)
    ...
    for index, event in enumerate(events):
        ...
        if isinstance(task_id, str) and task_id and task_id not in seen:
            issues.append({"index": index, "task_id": task_id, "reason": "runtime event references unknown task_id with no seed/bootstrap/PUBLISH/taskbook record"})
    return tasks, issues


def state_from_task(task: dict[str, Any]) -> dict[str, Any]:
    initial_orphan = task.get("initial_orphan_record")
    orphan_history = [copy.deepcopy(initial_orphan)] if isinstance(initial_orphan, dict) else []
    return {
        "task_id": task["task_id"], "state": task["base_state"], "claim_id": None,
        "claim_schema": None, "actor": None, "researcher_id": None,
        "last_researcher_id": None, "last_executor_id": None, "last_executor_actor": None,
        "identity_source": None, "lease_until": None, "hard_block": copy.deepcopy(task.get("hard_block")),
        ...
        "review_history": [], "orphan_history": orphan_history,
...
def _orphan_record(state: dict[str, Any], *, orphaned_at: datetime, reason: str, extra: dict[str, Any] | None = None) -> dict[str, Any]:
    record = {
        "orphaned_at": orphaned_at.isoformat(), "reason": reason,
        "claim_id": state.get("claim_id"), "actor": state.get("actor"),
        "researcher_id": state.get("researcher_id") or state.get("last_researcher_id"),
...
def expire_claim(state: dict[str, Any], at: datetime) -> None:
    ...
    if not state.get("claim_id") or not isinstance(lease_until, datetime) or at < lease_until:
        return
    legacy = state.get("claim_schema") == V1_EVENT_SCHEMA
    reason = "LEGACY_V1_LEASE_EXPIRED" if legacy else "LEASE_EXPIRED"
    state["orphan_history"].append(_orphan_record(
        state, orphaned_at=lease_until, reason=reason,
        extra={"derived": True, "legacy_auto_handoff": legacy},
    ))
    state["state"] = "HANDOFF_READY" if legacy else "ORPHANED"
    _clear_claim(state)
...
def _cross_review_error(state: dict[str, Any], stage: str, reviewer: str) -> str | None:
    reviewer = reviewer.strip().upper()
    if stage == "DISPATCH":
...
def _review_record(event: dict[str, Any], reviewer: str) -> dict[str, Any]:
    return {
        "at": event.get("at"), "stage": event.get("review_stage"),
        "verdict": event.get("verdict"), "reviewer_id": reviewer,
...
def reduce_task(task: dict[str, Any], events: Iterable[dict[str, Any]], *, default_lease_minutes: int, now: datetime) -> dict[str, Any]:
    ...
    for index, event in enumerate(matching):
        ...
        if kind == "CLAIM":
            ...
            if supplied is not None and not valid_researcher_id(supplied):
                ignore(state, index, "CLAIM researcher_id has invalid format")
                continue
            researcher = supplied.strip().upper() if isinstance(supplied, str) else researcher_id_for_claim(task, claim_id)
            try:
                ...
                continue
            state.update({
                "state": "CLAIMED", "claim_id": claim_id, "claim_schema": schema,
                "actor": event.get("actor"), "researcher_id": researcher,
                "last_researcher_id": researcher, "last_executor_id": researcher,
                "last_executor_actor": event.get("actor"),
                "identity_source": "EVENT" if supplied is not None else "AUTO_CLAIM_DERIVED",
        ...
        if kind in {"HEARTBEAT", "PROGRESS", "HANDOFF", "HARD_BLOCK", "RETURN"} or (kind == "DONE" and schema == V1_EVENT_SCHEMA):
            ...
            if supplied is not None:
                if not valid_researcher_id(supplied):
                    ignore(state, index, f"{kind} researcher_id has invalid format")
                    continue
                if supplied.strip().upper() != state.get("researcher_id"):
        ...
        if kind == "HANDOFF":
            ...
            state["last_progress_ref"] = event.get("progress_ref") or event.get("source_ref") or state.get("last_progress_ref")
            state["last_progress_at"] = event["at"]
        ...
        if kind == "RETURN":
            if schema != V2_EVENT_SCHEMA:
                ...
                continue
            ref = event.get("return_ref") or event.get("progress_ref")
            if not isinstance(ref, str) or not ref.strip():
                ignore(state, index, "RETURN requires return_ref or progress_ref")
                continue
            state["state"] = "RETURNED"
            state["return_ref"] = ref
            state["last_progress_ref"] = ref
            state["last_progress_at"] = event["at"]
            if event.get("next_action"):
                state["next_action"] = event["next_action"]
            _clear_claim(state)
        ...
        if kind == "HARD_BLOCK":
            block = event.get("hard_block")
            if not complete_hard_block(block):
                ignore(state, index, "HARD_BLOCK requires all four hard-block fields")
                continue
            state["state"] = "BLOCKED"
            state["hard_block"] = copy.deepcopy(block)
            state["last_progress_at"] = event["at"]
            if event.get("progress_ref"):
                state["last_progress_ref"] = event["progress_ref"]
            _clear_claim(state)
        ...
        if kind == "ORPHAN":
            ...
            state["orphan_history"].append(_orphan_record(
                ...
                extra={
                    "derived": False, "discovered_by": event["discovered_by"],
                    "source_ref": event["source_ref"], "branch": event.get("branch"),
                    "last_commit": event.get("last_commit"), "driver_id": event.get("driver_id"),
                },
            ))
        ...
        if kind == "RECOVER":
            if schema != V2_EVENT_SCHEMA or state["state"] != "ORPHANED":
                ignore(state, index, "RECOVER requires V2 schema and ORPHANED state")
                continue
            driver = event.get("driver_id")
            ref = event.get("review_ref")
            next_action = event.get("next_action")
            if not valid_driver_id(driver):
                ignore(state, index, "RECOVER requires valid driver_id")
                continue
            if not isinstance(ref, str) or not ref.strip() or not isinstance(next_action, str) or not next_action.strip():
                ignore(state, index, "RECOVER requires review_ref and next_action")
                continue
            reviewer = driver.strip().upper()
            state["review_history"].append({
                "at": event.get("at"), "stage": "RECOVERY", "verdict": "ACCEPT",
                "reviewer_id": reviewer, "review_ref": ref, "note": event.get("review_note"),
            })
            state["state"] = "HANDOFF_READY"
            state["next_action"] = next_action
            state["last_progress_at"] = event["at"]
        ...
        if kind == "REVIEW":
            ...
            if cross_error:
                ...
                continue
            expected = {"DISPATCH": "PUBLISHED", "RETURN": "RETURNED", "RECOVERY": "ORPHANED"}[stage]
            if state["state"] != expected:
                ignore(state, index, f"{stage} review requires {expected} state")
                continue
            if stage == "RETURN" and verdict == "REJECT":
                ignore(state, index, "RETURN review uses CHANGES_REQUESTED rather than REJECT")
                continue
            if stage == "DISPATCH" and verdict == "ACCEPT" and state.get("taskbook_ref"):
                policy_state = event.get("taskbook_policy_state") or state.get("taskbook_policy_state")
                if policy_state != "PASS":
                    ignore(state, index, "DISPATCH ACCEPT for a taskbook requires taskbook_policy_state=PASS")
                    continue
                state["taskbook_policy_state"] = policy_state
            if verdict == "ACCEPT":
                patch_error = _apply_task_patch(state, event)
                if patch_error:
                    ignore(state, index, patch_error)
                    continue
            state["review_history"].append(_review_record(event, reviewer))
        ...
        if kind == "DONE":
            if schema != V1_EVENT_SCHEMA:
                ...
                continue
            state["state"] = "DONE"
            state["last_progress_ref"] = event.get("progress_ref") or state.get("last_progress_ref")
            state["last_progress_at"] = event["at"]
            state["review_history"].append({
                "at": event.get("at"), "stage": "LEGACY_V1_DONE", "verdict": "GRANDFATHERED",
...
def effective_states(config: dict[str, Any], events: list[dict[str, Any]], now: datetime, *, root: pathlib.Path = ROOT) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    default_lease = int(config.get("claim_lease_minutes", 120))
    tasks, issues = collect_tasks(config, events, root=root)
    return [reduce_task(task, events, default_lease_minutes=default_lease, now=now) for task in tasks], issues


def select_task(config: dict[str, Any], events: list[dict[str, Any]], now: datetime, *, kind: str = "RESEARCH", root: pathlib.Path = ROOT) -> dict[str, Any] | None:
    policy = config["selection_policy"]
    states, _ = effective_states(config, events, now, root=root)
    ...
    candidates = [s for s in states if s["dispatch_state"] == "NEEDS_DISPATCH" and (kind == "ANY" or s["kind"] == kind)]
    if not candidates:
        return None
    def key(state: dict[str, Any]) -> tuple[Any, ...]:
        try:
            progress = parse_time(state["last_progress_at"])
        except (TypeError, ValueError):
            progress = datetime(1970, 1, 1, tzinfo=timezone.utc)
        return (
...
def registry_integrity(config: dict[str, Any], events: list[dict[str, Any]], *, root: pathlib.Path = ROOT) -> list[str]:
    ...
    tasks, issues = collect_tasks(config, events, root=root)
    for item in issues:
        if item.get("reason") == "duplicate PUBLISH task_id":
            errors.append(f"registry event[{item.get('index')}]: {item['reason']}: {item.get('task_id')}")
        elif str(item.get("reason", "")).startswith("runtime event references unknown") or str(item.get("reason", "")).startswith("malformed") or item.get("reason") == "duplicate taskbook task_id":
            errors.append(f"registry: {item.get('reason')}: {item.get('task_id') or item.get('source_ref')}")
    registered = {task["task_id"] for task in tasks}
    directory = root / config.get("registry_integrity", {}).get("taskbook_directory", "research_tasks")
    if directory.exists():
        for path in sorted(directory.glob("*.md")):
            text = path.read_text(encoding="utf-8")
            if not text.startswith(TASKBOOK_PREFIX):
            ...
            try:
                ...
                continue
            task_id = meta.get("task_id")
            if isinstance(task_id, str) and task_id not in registered:
                errors.append(f"{path.relative_to(root)}: task {task_id} is invisible to scheduler registry")
    for task in tasks:
        ref = task.get("taskbook_ref")
        if isinstance(ref, str) and ref.strip() and not (root / ref).exists():
            errors.append(f"{task['task_id']}: registered taskbook_ref does not exist: {ref}")
    return errors


def validate_effective_owners(config: dict[str, Any], owners: dict[str, Any], events: list[dict[str, Any]], now: datetime, *, root: pathlib.Path = ROOT) -> list[str]:
    errors: list[str] = []
    _, active = _owner_sets(owners)
    states, issues = effective_states(config, events, now, root=root)
    errors.extend(
        f"registry event[{item.get('index')}]: {item['reason']}"
        for item in issues if str(item.get("reason", "")).startswith("runtime event references unknown")
    )
    for state in states:
        if state["kind"] != "RESEARCH" or state["state"] in {"PUBLISHED", "ORPHANED"} | TERMINAL_STATES:
            continue
        if state.get("owner") not in active:
            errors.append(f"{state['task_id']}: effective executable owner is not ACTIVE_OWNER/ACTIVE_BRIDGE: {state.get('owner')!r}")
    return errors
...
def emit_publish(args: argparse.Namespace) -> int:
    ...
    fields = [
        "title", "kind", "owner", "priority", "leverage", "frontier", "next_action",
        "dependencies", "source_refs", "evidence_status", "tags", "identity_lane",
    ]
...
def emit_review(args: argparse.Namespace) -> int:
    ...
    event.update({
        "review_stage": args.review_stage, "verdict": args.verdict,
        "reviewer_id": args.reviewer_id, "review_ref": args.review_ref,
    })
    if args.review_note:
...
def emit_orphan(args: argparse.Namespace) -> int:
    ...
    for field in ("claim_id", "driver_id", "branch", "last_commit"):
...
def build_parser() -> argparse.ArgumentParser:
    ...
    select.add_argument("--events", type=pathlib.Path, required=True, help="exported append-only Issue #240 V1/V2 event stream")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "identity":
        ...
        print_json({
            "task_id": args.task_id, "claim_id": args.claim_id,
    ...
    if args.command == "registry-integrity":
        ...
        if issues:
            for issue in issues:
                print(f"ERROR: {issue}")
            return 1
        states, _ = effective_states(config, events, current)
        orphan_count = sum(state["state"] == "ORPHANED" for state in states)
        discovered_count = sum(state["registry_source"] == "TASKBOOK_ORPHAN_DISCOVERY" for state in states)
        print(f"PASS: scheduler registry integrity valid; all taskbooks are visible. ORPHANED={orphan_count}, auto-discovered={discovered_count}.")
        return 0
