...
def build_lab(out: str | Path, *, count: int = MAX_COUNT) -> Path:
    ...
    from .certified_hex_browser import SCRIPT as CELL_SCRIPT
    payload=make_payload(count)
    encoded=json.dumps(payload,ensure_ascii=False,separators=(',',':'),allow_nan=False).replace('<','\\u003c')
    text=TEMPLATE.replace('__ANGULAR_EXACT_SCRIPT__',SCRIPT).replace('__CERTIFIED_HEX_SCRIPT__',CELL_SCRIPT).replace('__LAB_PAYLOAD__',encoded)
...
<a href="multiplication.html#golden" target="lab">候选乘法布局</a><a href="multiplication.html#legacy" target="lab">原六角编码</a><a href="multiplication.html#zero" target="lab">全零相位反例</a><a href="multiplication.html#layers" target="lab">立体分层</a><a href="multiplication.html" target="_blank" rel="noopener">独立打开工作台</a><span id="galleryStatus" data-state="LOADING" aria-live="polite">工作台加载中</span></nav><iframe name="lab" title="数场交互工作台" ></iframe><script id="embedded" type="application/json">__EMBEDDED_LAB__</script><script>
'use strict';
const frame=document.querySelector('iframe'),statusNode=document.getElementById('galleryStatus');let selected='golden',generation=0,galleryState='LOADING';const code=JSON.parse(document.getElementById('embedded').textContent);
function setGalleryState(state,text){galleryState=state;statusNode.dataset.state=state;statusNode.textContent=text;}
async function selectPreset(name){selected=name;const token=++generation,api=frame.contentWindow&&frame.contentWindow.NumberFieldLab;if(!api){if(token===generation)setGalleryState('LOADING','工作台加载中');return null;}setGalleryState('COMPUTING','正在切换：'+name);try{await api.preset(name);if(token!==generation)return null;const state=api.status();setGalleryState('READY','已就绪：'+name);return state;}catch(error){if(token===generation)setGalleryState('FAILED','切换失败：'+error.message);throw error;}}
frame.addEventListener('load',()=>{void selectPreset(selected).catch(()=>{});});frame.srcdoc=code;for(const a of document.querySelectorAll('a[target="lab"]'))a.addEventListener('click',async e=>{e.preventDefault();try{await selectPreset(a.getAttribute('href').split('#')[1]);}catch(_error){}});window.NumberFieldGallery={selectPreset,status:()=>({selected,generation,state:galleryState,lab:frame.contentWindow&&frame.contentWindow.NumberFieldLab?frame.contentWindow.NumberFieldLab.status():null})};
</script></body></html>"""
