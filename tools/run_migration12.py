#!/usr/bin/env python3
"""Reproduce the M12 machine-field source slice; no installation or remote execution."""
...
def main():
    ...
    parser.add_argument('--out',type=Path,default=ROOT/'evidence/migration12/tests.json')
    ...
    names=('test_multiplication_lab','test_angular_dispersion','test_certified_hex',
           ...
           'test_multiplicative_readouts','test_multiplicative_machine')
    modules=[load(ROOT/'tools/nollm_visual_toolkit/tests'/f'{name}.py','m12_'+name) for name in names]
    ...
    record={'schema':'M12_MACHINE_FIELD_VALIDATION_V1','tests':result.testsRun,
            'new_machine_tests':counts['test_multiplicative_machine'],
            'failures':len(result.failures),'errors':len(result.errors),'skips':len(result.skipped),
            'groups':counts,'finite_cases':{**modules[-2].COUNTS,**modules[-1].COUNTS},'policy_exit':policy.returncode,
            'scope':'Machine field/config/export/CLI source slice and retained arithmetic regressions; no full M10 startup/DOM, package, navigation or independent review',
            'm11_expectations_advanced':2,
