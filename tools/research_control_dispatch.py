...
try:
    ...
    import research_lane_dispatch  # type: ignore
...
def parse_session_observations(value: Mapping[str, Any] | None) -> dict[str, str]:
    ...
    for index, row in enumerate(rows):
        ...
        # Parse now so malformed liveness evidence cannot silently affect routing.
    ...
    *,
    ...
    for target in leased_targets:
        ...
        if action == research_runtime.ADOPT_OWNER_CLAIM:
            ...
            # A state reduced as LEASED at the same `now` must not simultaneously
            # classify as unowned.  Fail closed rather than manufacture a claim.
            raise ControlDispatchError(
