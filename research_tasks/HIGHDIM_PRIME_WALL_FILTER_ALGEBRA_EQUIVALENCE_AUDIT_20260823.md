...
A free 2D–19D experiment produced candidate support-spectrum identities, a lambda-filter family, dimension convolution, four- and eight-dimensional primality walls, and higher-dimensional residual observations.

Several components visibly meet classical theta-series and divisor-sum theory. The present task is therefore not another numerical sweep. It is an exact independent derivation followed by a disciplined equivalence audit that prevents classical formulas from being relabeled as new Enterprise mathematics while preserving any genuinely useful project-specific transform or decomposition.
...
Before the internal proof checkpoint is frozen, do not read branch `research/free-highdim-prime-collapse-basins-20260823`, its scripts or notes, or the source GLOBAL_KNOWLEDGE event.
...
It includes exact candidate formulas so that theorem strength can be tested, but it excludes source derivations, code, interpolation choices, and source interpretations.
...
- exact coefficient and generating-function proofs for support decomposition and dimension convolution;
...
- identification of any quotient or residual object not exhausted by classical formulas.
