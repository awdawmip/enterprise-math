...
void analyze(int n,unordered_set<string>&all,ofstream&out,bool release_all=false){
    ...
    for(int i=0;i<(int)valid.size();i++){
        ...
        struct BC { vector<P> cp; P u; P vv; };
        long long bestDelta=0; bool have=false; vector<BC> bestCands; bestCands.reserve(8);
        for(int ui=0;ui<n;ui++){
            P u=v[ui]; bool bd=false;for(auto d:D)if(!occ.count(pack(addp(u,d)))){bd=true;break;} if(!bd)continue;
            for(auto d:D){P vv=addp(u,d);if(occ.count(pack(vv)))continue;
                ...
                if(!connectedv(cp) || !holefree(cp)) continue;
                if(!have||delta<bestDelta){have=true;bestDelta=delta;bestCands.clear();bestCands.push_back({std::move(cp),u,vv});}
                else if(delta==bestDelta){bestCands.push_back({std::move(cp),u,vv});}
            }
        }
        if(!have)islocal[i]=1;else{
            string bestMin,bestMax,bestT2; bool first=true; P t2u{999,999},t2v{999,999};
            for(auto &bc:bestCands){string ck=canon(bc.cp); if(first||ck<bestMin)bestMin=ck; if(first||ck>bestMax)bestMax=ck; if(first||make_pair(bc.u,bc.vv)<make_pair(t2u,t2v)){t2u=bc.u;t2v=bc.vv;bestT2=ck;} first=false;}
            succ[i].t0=findidx(bestMin);succ[i].t1=findidx(bestMax);succ[i].t2=findidx(bestT2);
        }
    }
    for(int i=0;i<(int)valid.size();i++) if(islocal[i]) locals.push_back(i);
...
int main(int argc,char**argv){int maxn=12;if(argc>1)maxn=stoi(argv[1]);string outpath=argc>2?argv[2]:"r055_exhaustive.tsv";ofstream out(outpath);out<<"# R055 exhaustive exact analysis\n";unordered_set<string>cur,nxt;cur.insert(string("\0\0",2));analyze(1,cur,out,maxn==1);for(int n=2;n<=maxn;n++){auto t=chrono::steady_clock::now();nxt.clear();nxt.reserve(cur.size()*5);for(auto const&key:cur){auto v=decode(key);auto f=frontier(v);for(auto z:f){auto w=v;w.push_back(z);nxt.insert(canon(w));}}cerr<<"generated n="<<n<<" total="<<nxt.size()<<" sec="<<chrono::duration<double>(chrono::steady_clock::now()-t).count()<<"\n"<<flush;analyze(n,nxt,out,n==maxn);cur.swap(nxt);} }
