...
void analyze(int n,unordered_set<string>&all,ofstream&out,bool release_all=false){
    ...
    vector<string> valid;valid.reserve(all.size());for(auto const&k:all)if(holefree(decode(k)))valid.push_back(k);sort(valid.begin(),valid.end());
    if(release_all){all.clear();all.rehash(0);}
    auto findidx=[&](const string&k){auto it=lower_bound(valid.begin(),valid.end(),k);if(it==valid.end()||*it!=k)throw runtime_error("missing successor");return (int)(it-valid.begin());};
    vector<long long> G(valid.size());vector<int>Pedge(valid.size());long long gmin=LLONG_MAX;int pmin=INT_MAX;
    #pragma omp parallel for schedule(static)
    for(int i=0;i<(int)valid.size();i++){auto v=decode(valid[i]);G[i]=energy(v);Pedge[i]=pedge(v);}
    for(int i=0;i<(int)valid.size();i++){gmin=min(gmin,G[i]);pmin=min(pmin,Pedge[i]);}
    vector<int> gmins,pmins;for(int i=0;i<(int)valid.size();i++){if(G[i]==gmin)gmins.push_back(i);if(Pedge[i]==pmin)pmins.push_back(i);}
    vector<Succs>succ(valid.size());vector<unsigned char>islocal(valid.size(),0);
    ...
    for(int i=0;i<(int)valid.size();i++){
        ...
        bool have=false;string bestMin,bestMax,bestT2;size_t pos=0;
        while(pos<rms.size()&&!have){size_t end=pos+1;while(end<rms.size()&&rms[end].delta==rms[pos].delta)end++;bool first=true;P t2u{999,999},t2v{999,999};
            for(size_t j=pos;j<end;j++){auto &m=rms[j];vector<P>cp=v;cp[m.ui]=m.vv;string raw=raw_norm(cp);if(raw==valid[i])continue;if(!connectedv(cp)||!holefree(cp))continue;string ck=canon(cp);if(first||ck<bestMin)bestMin=ck;if(first||ck>bestMax)bestMax=ck;if(first||make_pair(m.u,m.vv)<make_pair(t2u,t2v)){t2u=m.u;t2v=m.vv;bestT2=ck;}first=false;have=true;}
            if(!have)pos=end;
        }
        if(!have)islocal[i]=1;else{succ[i].t0=findidx(bestMin);succ[i].t1=findidx(bestMax);succ[i].t2=findidx(bestT2);}
    }
    vector<int>locals;for(int i=0;i<(int)valid.size();i++)if(islocal[i])locals.push_back(i);
    vector<char>d2min(valid.size(),0);int localNotD2=0;for(int i:locals){bool imp=d2_has_improvement(decode(valid[i]),valid,G[i]);d2min[i]=!imp;if(imp)localNotD2++;}
    ...
    out<<"SUMMARY\t"<<n<<"\t"<<valid.size()<<"\t"<<gmin<<"\t"<<gmins.size()<<"\t"<<pmin<<"\t"<<pmins.size()<<"\t"<<overlap<<"\t"<<locals.size()<<"\t"<<localNotD2<<"\t"<<sec<<"\n";
