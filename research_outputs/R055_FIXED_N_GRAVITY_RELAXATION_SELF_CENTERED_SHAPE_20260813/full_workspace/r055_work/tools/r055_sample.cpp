...
void analyze(int n,unordered_set<string>&all,ofstream&out,bool release_all=false){
    ...
    if(n==12 && valid.size()>20000) valid.resize(20000);
    ...
    for(int i=0;i<(int)valid.size();i++){
        ...
        if(!have)islocal[i]=1;else{succ[i].t0=0;succ[i].t1=0;succ[i].t2=0;}
    }
    ...
    vector<char>d2min(valid.size(),1);int localNotD2=0;
