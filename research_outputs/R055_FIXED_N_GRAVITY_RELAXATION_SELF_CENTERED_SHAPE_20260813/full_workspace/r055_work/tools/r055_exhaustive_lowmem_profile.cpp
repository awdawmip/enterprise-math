...
void analyze(int n,unordered_set<string>&all,ofstream&out,bool release_all=false){
    ...
    vector<string> valid;valid.reserve(all.size());for(auto const&k:all)if(holefree(decode(k)))valid.push_back(k);sort(valid.begin(),valid.end()); cerr<<" phase filter n="<<n<<" t="<<chrono::duration<double>(chrono::steady_clock::now()-t0).count()<<"\n"<<flush;
    if(release_all){all.clear(); all.rehash(0);} cerr<<" phase release n="<<n<<" t="<<chrono::duration<double>(chrono::steady_clock::now()-t0).count()<<"\n"<<flush;
    ...
    vector<int> gmins,pmins;for(int i=0;i<(int)valid.size();i++){if(G[i]==gmin)gmins.push_back(i);if(Pedge[i]==pmin)pmins.push_back(i);} cerr<<" phase metrics n="<<n<<" t="<<chrono::duration<double>(chrono::steady_clock::now()-t0).count()<<"\n"<<flush;
    ...
    for(int i=0;i<(int)valid.size();i++) if(islocal[i]) locals.push_back(i); cerr<<" phase d1 n="<<n<<" local="<<locals.size()<<" t="<<chrono::duration<double>(chrono::steady_clock::now()-t0).count()<<"\n"<<flush;
    ...
    cerr<<" phase d2 n="<<n<<" t="<<chrono::duration<double>(chrono::steady_clock::now()-t0).count()<<"\n"<<flush;
