...
void analyze(int n,unordered_set<string>&all,ofstream&out,bool release_all=false){
    ...
    for(int i=0;i<(int)valid.size();i++){
        auto v=decode(valid[i]);auto occ=pointset(v);P S{0,0};for(auto p:v){S.a+=p.a;S.b+=p.b;} bool imp=false;
        for(int ui=0;ui<n && !imp;ui++){
            P u=v[ui];bool bd=false;for(auto d:D)if(!occ.count(pack(addp(u,d)))){bd=true;break;}if(!bd)continue;
            for(auto d:D){P vv=addp(u,d);if(occ.count(pack(vv)))continue;int touch=0;for(auto d2:D){P q=addp(vv,d2);if(q==u)continue;if(occ.count(pack(q)))touch++;}if(touch==0)continue;
                P dd=subp(vv,u);long long delta=(long long)n*(Q(vv)-Q(u))-(L(S,dd)+Q(dd));if(delta>=0)continue;vector<P>cp=v;cp[ui]=vv;string raw=raw_norm(cp);if(raw==valid[i])continue;
                if(connectedv(cp)&&holefree(cp)){imp=true;break;}
            }
        }
        if(!imp)islocal[i]=1;
    }
    ...
    out.flush();cerr<<"N"<<n<<" stats local="<<locals.size()<<" sec="<<sec<<"\n"<<flush;
}
