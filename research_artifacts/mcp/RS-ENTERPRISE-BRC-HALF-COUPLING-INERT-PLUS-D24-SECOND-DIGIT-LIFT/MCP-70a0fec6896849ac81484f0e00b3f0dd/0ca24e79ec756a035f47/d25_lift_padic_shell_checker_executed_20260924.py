...
def verify_prime(p):
    ...
    mod=p**3
    t=terms_mod_p3(p)
    # Shell valuations: v_p(C_k)=floor(2k/p)+floor(3k/p).
    # The observable weight can only add valuation, with the unique p-multiple at k=m.
    ...
    # Third shell (k=3m+r) after division by p^2.
    ...
    # Equivalent truncated 5F4 shell sum, generated by the same universal ratio.
    q=f1
    K=q
    for r in range(1,m):
        num=r*(6*r-1)*(6*r+1)*(3*r+2)
