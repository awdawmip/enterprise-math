...
For the canonical old signed retraction
...
A pre-erasure two-alternative state is an ordered pair

`(x,y) in C2 ⊕ C2`.

Marker names are presentation labels. Slot swap

`P(x,y)=(y,x)`

is an allowed presentation relabeling.
...
F7 may introduce an additive automorphism

`M in Aut(C2 ⊕ C2)`

representing a reversible local two-slot mixing/refinement.

The new operational requirements are exactly:

1. **global marked conservation**:
   `Q(M(x,y))=Q(x,y)` for every marked pair in the declared operation domain;
2. **elementary balance**: if
   `M(e,0)=(u,v)`, then
   `q(u)=q(v)=1/2`;
3. **elementary A0**:
   `pi(u)!=0` and `pi(v)!=0`;
4. **genuine two-slot mixing**:
   the elementary input has nonzero outputs in both marked slots, and `M` is not physically equivalent to a product of independent unary slot automorphisms or such a product composed only with marker swap;
5. **reversibility**: exact inverse exists because `M` is an additive automorphism.

Do not silently impose `MP=PM`, a Hadamard relation, an orthogonality relation, a chosen matrix shape, or commutation with every inherited unary map. If any additional covariance relation is claimed necessary, derive it from the declared semantics and state it separately.

## 6. Free quotient bookkeeping

Modulo torsion, each slot has free coordinates `(e,f)`, so the two-slot free quotient is `Z^4`.

Write the free block of `M` as
...
For every successful model class found or structurally permitted, determine whether the new free direction `f` is:

- necessarily active in the elementary split;
- optionally active;
- or can remain completely spectator while the full rank-two carrier is still required elsewhere.

Prove the strongest exact statement available. Do not infer dynamic use of `f` merely from the rank lower bound.
...
Derive necessary structural conditions on the `GL_4(Z)` free block and on its elementary first column from additivity, invertibility, A0, balance, scalar conservation, unary invariance and free-projection positivity.

Give exact sufficient families where possible. Distinguish theorem-level constraints from bounded census observations.

Do not claim an exhaustive arbitrary-`GL_4(Z)` membership theorem unless actually proved.
...
Classify what scalar information is actually forced at F7 scope.

At minimum determine:

- whether an exact conserved scalar exists for each constructed free-block family;
- whether the scalar law is unique;
- whether two inequivalent exact scalar laws or two inequivalent exact mixing classes survive the same frozen axioms;
- whether support/periodic/pathological laws remain possible despite free-projection positivity;
- whether any additional regularity would be required for uniqueness.

If strict underdetermination remains, prove it by explicit physically inequivalent exact models, not by parameter counting alone.
...
Use only target-independent equivalences:

- marker relabeling;
- operation orientation `M<->M^-1`;
- typed carrier automorphisms preserving the accepted F6 unary class and old projection.

Classify the exact equivalence of every returned witness family. If a minimum-complexity representative is discussed, declare the complexity order before using it and do not treat a minimizer as physically selected unless the axioms select that order.
...
A strict-underdetermination theorem is a successful completion if proved exactly.

## 9. What F7 must NOT do

Do not:

- read historical F3/F3R/F3R2 mixing reports before raw freeze;
- read R063/R064/R065/FQ or downstream coherent-wave work;
- import external quantum mechanics or known beam-splitter constructions;
- identify `e,f` with real/imaginary coordinates;
- introduce complex/Gaussian/quadratic carriers;
- introduce rings, coefficient multiplication, roots of unity or phase groups;
- assume a norm, inner product, square law, p-norm, polarization or Born rule;
- assume Hadamard/Fourier matrices;
- fit a continuum/wave equation;
- perform a full arbitrary torsion-lift membership completion unless it follows cheaply from a proved reduction.

## 10. Deterministic evidence expectation

A successor taskbook will require a checker that, at minimum:

- verifies all exact witness matrices are integral automorphisms;
- verifies declared physical-equivalence relations;
- verifies elementary A0 and balance witnesses;
- verifies exact conservation for every finite quotient/reduction used in a proof;
- verifies unary-invariance identities used by the scalar construction;
- searches a bounded `GL_4(Z)` window only as regression/counterexample discovery, never as completeness proof;
- reports zero theorem/model mismatch.
