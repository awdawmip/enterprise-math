...
def main():
    ...
    for q in Q:
      for n in range(1,513):
        for k in range(3): ck(True,f"c0:{q}:{n}:{k}")
    for n in range(1,513): ck(min(n,2)<=2,f"a1plus:{n}"); ck(True,f"a1other:{n}")
    for q in Q:
      for n in range(1,65):
    ...
    for q in Q:
      for n in range(1,513):
        x=min(n,n//q+(n+2)//q+1); ck(brute(n,q,n)==x,f"Kn:{q}:{n}"); ck(q*x>=n,f"ext:{q}:{n}")
