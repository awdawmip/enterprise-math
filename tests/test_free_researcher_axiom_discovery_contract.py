...
def test_free_researcher_is_axiom_discovery_not_waiting_queue_worker():
    ...
    assert "enter:\n\n`AXIOM_DISCOVERY`" in role
    assert "not `WAITING_FOR_TOPIC`" in role
    assert "does **not** apply to this role" in role


def test_phase_a_blocks_recent_route_and_scheduler_anchoring():
    role = ROLE.read_text(encoding="utf-8")
    ...
    assert "R063_IS_NOT_DEFAULT_CONTEXT" in anti
