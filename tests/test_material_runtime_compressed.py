...
class MaterialRuntimeCompressedTests(unittest.TestCase):
    def test_run_encoding_is_lossless_for_every_normalized_cell(self):
        ...
        self.assertEqual(compressed.max_cell, 128)
    ...
    def test_high_precision_treloar_curve_compresses_to_expected_size(self):
        ...
        for amplitude, root_power, hard_power, scale, runs, packed_bytes in cases:
            ...
            self.assertEqual(compressed.max_cell, amplitude)
    ...
    def test_generic_profile_branch_compresses_without_using_profile_amplitude_as_cell_count(self):
        ...
        compressed = compress_monotone_material_curve(dense)
        self.assertEqual(dense.max_cell, 4)
        self.assertEqual(compressed.max_cell, 4)
        for cell, expected in enumerate(profile.loading):
            self.assertEqual(lookup_run_material_cell(compressed, cell), expected)
        self.assertEqual(
            lookup_run_material_deformation(compressed, 200),
            profile.loading[2],
        )
