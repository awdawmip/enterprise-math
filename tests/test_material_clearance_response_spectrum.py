...
from enterprise_math.material_clearance_response_spectrum import (
    material_clearance_coverage,
    material_clearance_response_spectrum,
)
...
class MaterialClearanceResponseSpectrumTests(unittest.TestCase):
    def test_closed_form_coverage_matches_direct_clearance_enumeration(self):
    ...
    def test_full_material_depth_covers_entire_coarse_only_box(self):
        for dimension in range(1, 5):
            for factor in range(1, 8):
                report = material_clearance_coverage(
                    dimension,
                ...
                self.assertEqual(
                    report.represented_states,
                    factor**dimension - 1,
                )

    def test_response_bins_match_direct_shell_to_curve_mapping(self):
        ...
        for dimension in (1, 2, 3):
            factor = 6
            report = material_clearance_response_spectrum(
                dimension, factor, profile, RETURNING
            )
            direct = Counter()
            underresolved = 0
            ...
            self.assertEqual(
                report.coverage.underresolved_states,
                underresolved,
            )

    def test_curve_plateaus_merge_multiple_spatial_shells_into_one_response_bin(self):
        # Return retention 0 maps every represented depth to response sample 0.
        profile = material_curve_profile(
            (0, 25, 50, 75, 100),
            amplitude=100,
            loading_power=1,
        ...
        report = material_clearance_response_spectrum(2, 5, profile, RETURNING)
    ...
    def test_loading_and_returning_can_induce_different_precision_space_spectra(self):
        profile = material_curve_profile(
            (0, 20, 40, 60, 80, 100),
            amplitude=100,
            loading_power=2,
        ...
        loading = material_clearance_response_spectrum(2, 6, profile, LOADING)
        returning = material_clearance_response_spectrum(2, 6, profile, RETURNING)
