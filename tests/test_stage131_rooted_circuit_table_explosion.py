...
class Stage131RootedCircuitTableExplosionTests(unittest.TestCase):
    ...
    def test_correct_exact_small_width_polynomials(self):
        self.assertEqual(rooted_circuit_width_polynomial(1), {2: 1})
        self.assertEqual(rooted_circuit_width_polynomial(2), {2: 1, 3: 2, 4: 1})
        self.assertEqual(
            rooted_circuit_width_polynomial(3),
            {2: 1, 3: 2, 4: 5, 5: 6, 6: 6, 7: 4, 8: 1},
        )
    ...
    def test_explicit_small_tree_enumeration_matches_generating_polynomial(self):
        for height in range(1, 5):
            ...
            self.assertEqual(enumerated_width_histogram(tree, tree.root), rooted_circuit_width_polynomial(height))
    ...
    def test_every_width_from_two_through_all_leaves_occurs(self):
        for height in range(1, 10):
            ...
            self.assertEqual(set(polynomial), set(range(2, (1 << height) + 1)))
    ...
    def test_correct_total_premise_literals_and_average_width(self):
        expected_literals = {
            1: 2,
            2: 12,
            3: 130,
        ...
        for height, literals in expected_literals.items():
            ...
            self.assertEqual(rooted_circuit_average_premise_width(height), Fraction(literals, rooted_circuit_count(height)))
    ...
    def test_height_five_report_uses_correct_premise_storage(self):
        ...
        self.assertEqual(report.root_total_premise_literals, 9224802)
        self.assertEqual(report.root_average_premise_width, Fraction(9224802, 458329))
