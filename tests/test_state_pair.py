    ...
    def test_critical_square_pair_is_diagonal_exactly_when_square_commutes(self):
        for power in range(2, 5):
            ...
            for ratio in range(1, 6):
                for state in range(100):
                    pair = critical_square_pair(operation, operation, state, ratio)
                    self.assertEqual(
                        on_diagonal(pair),
                        operation(state) // ratio == operation(state // ratio),
                    )
        ...
        for suffix in suffixes:
            for left in range(80):
                for right in range(80):
                    self.assertTrue(
                        kernel_monotonic_under_suffix(first, suffix, left, right)
                    )
