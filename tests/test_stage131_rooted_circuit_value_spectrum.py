...
class Stage131RootedCircuitValueSpectrumTests(unittest.TestCase):
    ...
    def test_width_marginal_matches_corrected_parent_polynomial(self):
        for height in range(1, 9):
            self.assertTrue(spectrum_marginals_match_parent_width_polynomial(height))
            self.assertEqual(spectrum_total_count(rooted_circuit_width_depth_spectrum(height)), rooted_circuit_count(height))
    ...
    def test_depth_cumulative_count_is_host_height_invariant(self):
        for host_height in range(1, 9):
            ...
            for depth in range(1, host_height + 1):
                ...
                self.assertEqual(cumulative, rooted_circuit_count(depth))
                self.assertEqual(rooted_circuit_count_through_base_depth(host_height, depth), rooted_circuit_count(depth))

    def test_exact_depth_counts_are_successive_circuit_count_differences(self):
        expected_h5 = {1: 1, 2: 3, 3: 21, 4: 651, 5: 457653}
        self.assertEqual(rooted_circuit_depth_count_closed(5), expected_h5)
        for host_height in range(1, 9):
            histogram = rooted_circuit_depth_histogram(host_height)
            for depth in range(1, host_height + 1):
                self.assertEqual(histogram[depth], rooted_circuit_exact_base_depth_count(host_height, depth))
    ...
    def test_every_exact_depth_has_dense_width_interval(self):
        for host_height in range(1, 8):
            ...
            for depth in range(1, host_height + 1):
                ...
                self.assertEqual((lower, upper), (depth + 1, 1 << depth))
                self.assertEqual(widths_at_exact_base_depth(host_height, depth), tuple(range(depth + 1, (1 << depth) + 1)))
    ...
    def test_height_five_opportunity_spectrum_is_dominated_by_deep_circuits(self):
        ...
        self.assertEqual(tuple(item.circuit_count for item in opportunities), (1, 3, 21, 651, 457653))
        deepest = opportunities[-1]
        self.assertEqual(deepest.one_round_saving, 4)
        self.assertEqual((deepest.min_premise_width, deepest.max_premise_width), (6, 32))
        self.assertEqual(deepest.share_of_all_root_circuits, Fraction(457653, 458329))
        self.assertGreater(deepest.share_of_all_root_circuits, Fraction(99, 100))
    ...
    def test_availability_direct_seed_is_always_depth_zero(self):
        for height in range(0, 8):
            self.assertEqual(availability_width_depth_spectrum(height)[(1, 0)], 1)
