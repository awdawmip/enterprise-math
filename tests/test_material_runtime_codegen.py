...
from enterprise_math.material_runtime import LOADING, compile_material_profile_branch, compile_root_basin_material_curve
...
class MaterialRuntimeCodegenTests(unittest.TestCase):
    def test_treloar_a128_header_has_exact_state_certificate_and_no_float_types(self):
        ...
        self.assertIn("TRELOAR_A128_MATERIAL_MAX_CELL 128u", header)
        ...
        self.assertNotIn("MATERIAL_AMPLITUDE", header)
    ...
    def test_high_precision_header_uses_16_bit_cell_and_value_fields(self):
        ...
        self.assertIn("TRELOAR_A8192_MATERIAL_MAX_CELL 8192u", header)
    ...
    def test_profile_codegen_uses_branch_cell_count_not_response_amplitude(self):
        profile = material_curve_profile((0, 25, 50, 75, 100), amplitude=100, loading_power=2)
        dense = compile_material_profile_branch(profile, 0, 400, LOADING)
        curve = compress_monotone_material_curve(dense)
        header = emit_c_run_material_header(curve, "profile_load")
        self.assertIn("PROFILE_LOAD_MATERIAL_MAX_CELL 4u", header)
        self.assertNotIn("PROFILE_LOAD_MATERIAL_MAX_CELL 100u", header)
