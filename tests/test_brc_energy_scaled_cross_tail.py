...
def test_scaled_cross_is_information_equivalent_to_mixed_coordinate() -> None:
    ...
    for _ in range(1000):
        ...
        scaled = scaled_cross_from_mixed(mixed, q1, q2, j1)
        assert scaled % 6 == 0
        assert mixed_from_scaled_cross(scaled, q1, q2, j1) == mixed


def test_scaled_cross_third_difference_matches_existing_identity() -> None:
    ...
    for _ in range(2000):
        ...
        mixed = s * u
        scaled = scaled_cross_from_mixed(mixed, s, a, t)
        q_new = q + s + a + e
        j_new = j + t + u + w
        ...
        compact = square_increment_scaled_cross_third_difference(
            target_root=j_new,
...
def test_scaled_cross_recurrence_is_exact() -> None:
    ...
    for _ in range(2000):
        ...
        u_new = u + w
        t_new = t + u_new
        direct = 6 * (s_new * u_new + a_new * (s_new + t_new))
        transported = next_scaled_cross(
            scaled_cross=scaled,
            quotient_first=s,
            root_first=t,
            quotient_third=e,
...
def _assert_same_state(
    ...
    compact: ScaledCrossEnergyTailScanner,
) -> None:
    assert (
        compact.multiplier,
        compact.root,
...
def test_exhaustive_small_scans_match_current_energy_jet() -> None:
    for n in range(1, 300, 2):
        ...
        compact = ScaledCrossEnergyTailScanner(n)
        for _ in range(120):
...
def test_seed_budgets_are_unchanged() -> None:
    scanner = ScaledCrossEnergyTailScanner((1 << 1023) + 987654321)
    for _ in range(15):
    ...
    for _ in range(200):
        ...
        assert scanner.last_energy_mode == "SCALED_CROSS_RECURRENCE"
    assert scanner.seed_divisions == 15
    assert scanner.seed_square_increment_products == 15
