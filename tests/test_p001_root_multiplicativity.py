...
from enterprise_math.core import integer_root
...
class RootMultiplicativityTests(unittest.TestCase):
            ...
            for a in range(0, 151):
                for b in range(0, 151):
                    self.assertGreaterEqual(
                        integer_root(a * b, p),
                        integer_root(a, p) * integer_root(b, p),
                    )
        ...
        for p in range(1, 6):
            for a in range(0, 121):
                for b in range(0, 121):
                    ...
                    multiplicative = integer_root(a * b, p) == r * s
    ...
    def test_one_perfect_factor_is_not_sufficient(self) -> None:
        self.assertEqual(integer_root(4, 2), 2)
        self.assertEqual(integer_root(3, 2), 1)
        self.assertEqual(integer_root(12, 2), 3)
