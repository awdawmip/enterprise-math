...
class CausalBoundaryTests(unittest.TestCase):
    ...
    def test_boundary_is_equivariant_under_graph_symmetry(self):
        ...
        transported = transported_boundary_complex(
            vertices, edges, expansion.__getitem__, rotation
        )
        transported_field = {
            rotation[vertex]: expansion[vertex]
            for vertex in vertices
        }
        recomputed = causal_boundary_complex(
            vertices, edges, transported_field.__getitem__
        )
