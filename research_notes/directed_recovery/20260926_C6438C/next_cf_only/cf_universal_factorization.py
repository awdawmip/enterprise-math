# Fixed K33 / original CF-only variant with an explicit all-width support bound.
...
def factor_integer_cf_universal(N, rng, *, failure_bits=16, max_attempts=None, base_provider=None):
    ...
    return {'schema': 'BRC_CF_UNIVERSAL_FACTORIZATION_V1', 'N': N,
...
def verify_factorization(result):
    ...
    if result.get('schema') != 'BRC_CF_UNIVERSAL_FACTORIZATION_V1': raise ValueError('wrong result type')
    N = result['N']
