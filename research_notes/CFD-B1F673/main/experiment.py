...
def main():
    ...
    ap.add_argument('--repeats',type=int,default=5); ap.add_argument('--resume',action='store_true'); args=ap.parse_args()
    ...
    if args.suite=='contracts': result['tests']=contract_tests()
        ...
        result['records']=[]
        if args.resume:
            for line in (here/('stdout_'+args.suite+'.jsonl')).read_text().splitlines():
                row=json.loads(line)
                if 'case' in row: result['records'].append(row)
            result['recovered_completed_cases']=len(result['records'])
            result['pre_checkpoint_runner_sha256']=sha(here/'experiment_pre_checkpoint.py')
        for case in frozen['cases']:
            if case['n']!=int(args.suite) or any(r['case']==case for r in result['records']): continue
