...
def context(n):
    ...
    s = spectralDNS.get_solver(parse_args=[
        '--optimization', 'numba', '--M', m, m, m, '--dealias', '3/2-rule', '--nu', '0.01',
...
def main():
    ...
    out=HERE/'host_validation_numba.json'
    report={'schema':'CFD_PINNED_NATIVE_HOST_VALIDATION_V1', 'task_id':TASK,
        ...
        'domain':[float(2*np.pi)]*3,'dealias':'3/2-rule','nyquist_mask':True,'native_optimization':'numba','upstream_fastmath_enabled':True,
        ...
        'cost_excluded':['initial shared input generation','dependency installation','first pair-kernel JIT warmup','explicit both-arm per-case native optimization warmup','disk output','report writing'],
        'rhs_cases':[],'trajectory_cases':[],'failures':[],
        'source_hashes':{p.name:hashlib.sha256(p.read_bytes()).hexdigest() for p in [HERE/'native_adapter.py',HERE/'host_validation_numba.py']}}
    def save():
    ...
    for n,name,seed in MATRIX:
        ...
        try:
            ...
            row={**key,'case_warmup_seconds':case_warmup_seconds,'comparisons':comparisons,'trials':trials,'median_total_seconds':tm,
...
if __name__=='__main__':
    try:
        ...
        (HERE/'host_validation_numba_fatal.txt').write_text(traceback.format_exc())
        raise
