"""Research prototype: positive BRC branches with exact affine effects.

NOT a production change or a newly admitted Foundation family.
...
try:
    ...
    if exc.name != "enterprise_math":
        raise
    from reference_brc.brc_histogram import WeightHistogram
...
class Affine:
    ...
    def homogeneous(self) -> Matrix:
        # The last 1 is an algebraic constant, NOT an added spatial axis.
...
class EffectHistogram:
    ...
    def forget_effects(self) -> WeightHistogram:
        """Use the exact existing BRC implementation, not a rewritten summary."""
        counts = {}
    ...
    def evaluate(self, x: Sequence[int | Fraction]) -> dict[Vector, WeightHistogram]:
        """Exact endpoint -> weight histogram; path labels are not reconstructed."""
        counts = {}
    ...
    def moment_action(self, moment: Matrix) -> Matrix:
        """Positive raw degree<=2 moments under fixed-weight affine branches.

        Independent next branch choice/constant kernel required. No Gaussian
        assumption, probability normalization or covariance-inverse claim.
        """
        n = self.dim+1
...
def factor_defect(a_m: Matrix, a_n: Matrix, a_mn: Matrix) -> Matrix:
    """D(m,n)=A_m A_n A_(mn)^(-1), a comparison map over Q.

    Do NOT interpret as a legal integer-lattice inverse on arbitrary inputs.
    """
    return mm(mm(a_m,a_n),inv(a_mn))


def euclidean_digits(x: int, bases: Sequence[int]) -> tuple[int, tuple[int,...]]:
    """T5 specialization: mixed-radix quotient and exact remainders, including x<0."""
...
@dataclass(frozen=True)
class MomentState:
    """Store only the independent symmetric homogeneous degree<=2 entries."""
    dimension: int
