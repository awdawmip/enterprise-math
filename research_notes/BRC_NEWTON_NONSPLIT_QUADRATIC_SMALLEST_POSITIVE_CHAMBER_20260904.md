...
Hence the full exact chamber is
...
## 4. Interval-root interpretation
...
This gives an independent exact validation route using the existing rational Sturm machinery.
...
For affine parameter forms \(a(\lambda),b(\lambda),r(\lambda)\) with \(r>0\), the chamber is a finite semi-algebraic Boolean combination of:

- the quadratic discriminant inequality \(D<0\) or \(D\ge0\);
- the sign of \(b\);
- the sign of \(R\);
- the affine order inequalities \(a\ge0\) and \(L>0\).
...
3. compare the closed piecewise formula with an independent exact Sturm count of roots in \((0,r)\);
