...
范围：P018-T182 之后的 quotient-path 平坦性与严格平方根尺度下降  
依赖：P018-T182、P007 离散除法，以及自然数 quotient 的规范恒等式 `Nat.div_div_eq_div_mul`  
纪律：floor division 的复合恒等式属于成熟算术，**不**声称为项目新数学。项目专门内容，是把该恒等式与 T182 结合以消除多步 root 分支的表面爆炸，并进一步得到平方盆地上的严格下降结论。

> **并发编号纠偏。** 本文由同一条并发 QuotientBasin 路线以临时编号 `Supplement 13 / T111–T112` 合入 `main`，而 PR #68 此前已经存在并验证了更早的 T111–T112。集成分支保留 #68 已建立的编号，只把这篇较晚的并发续篇重标为 **Supplement 22 / T195–T196**。数学内容与 Lean theorem names 均不改变。

## 1. 为什么 T182 不会产生二叉树式爆炸

T182 说明，一个平方盆地经过一次非平凡 quotient 后，最多只会碰到两个相邻 square-root indices。
...
这个恒等式本身属于前人算术；P018 的新增后果来自把它与 T182 结合。
...
因此两步 quotient **不会**产生四个最终 root-index 分支。两步先精确压平为一次除以 `ab`，再只需对总除数应用一次 T182。

同理，任意有限 quotient path 的最终 quotient state 只取决于总乘积 divisor，所以最终 square-root image 仍然只由一次 T182 的二盆地界控制。

这里并不声称不同因式分解下的中间状态都相同；结论只是在总除数固定时，最终 quotient projection 对 divisor 的因式分解是平坦的。

---

## 3. P018-T196 — k >= 3 时实际 quotient root 严格下降

状态：`PROVED`。

T182 给出 base root `j<k`，但形式上仍允许实际 quotient root 为 `j+1`；理论上存在 `j+1=k` 的边缘可能。
...
这是**实际运输后状态**的严格下降，而不只是 T182 的 base index 严格下降。
...
T182、T195、T196 现在给出一个很紧凑的 operation-level 图景。
...
- 用 quotient-path 平坦性证明反复 T182 transport 不会造成指数级最终 root-scale 分支；
- T196 的精确平方盆地严格 root descent；
...
形式化层有意只证明最小的两步路径恒等式及其 T182 后果。更长路径由普通归纳得到，除非后续应用确实需要，否则不扩成冗余 theorem family。

---

## 8. 下一目标

下一步真正有意义的问题已经不是证明 lower-band factor extraction 会下降；T182/T195/T196 已经提供了这个骨架。
