...
依赖：P017 L035、L039–L045  
纪律：**本文不证明勒让德猜想。** 本文不引入新的筛法形式。L041 处理 support closure，L042–L045 处理 centered mirror separation；这里的 L046 则把规范的大模数命中跨 least-factor shells 汇总。

## 1. 为什么仍需要另一条跨状态不等式

L042–L045 关联同一个 centered mirror pair 的两侧状态。L036 与 L040 则在单个 least-factor shell 内控制 three-prime states。它们都没有阻止同一个 cofactor resource prime 在另一个 least-factor shell 中再次被计数。
...
固定一个作为高带 triple state 两个 cofactor primes 之一的素数 `r`。

若 `n=p\ell s` 使用 `r`，则 `p<=r`。另一个 cofactor prime 至少为 `p`，所以
...
则 `r^2` 自动是 `p`-rough，所以 `p r^2` 确实是 least-factor-`p` shell 中的 three-prime state。
...
固定资源素数 `r`，令 `u_r(k)` 计数 cofactor support 中包含 `r` 的实际高带 triple states。

任取其中一个状态。其 least prime `p` 属于 `P_H(k)`。由于另一个 cofactor prime 至少为 `p`，
...
两个二值 modulus hits 实际只占一个 resource-19 state slot。这就是此前反例筛选后真正保留下来的共同中心现象：L046 **不**宣称 hit bits 在统计上普遍稀疏，只利用它们实现状态之间的精确碰撞。

---

## 7. 回归 checkpoint

可执行测试会在有限 `k` 范围内，把 L046 与真实 high-band triple states 直接对照。
...
已经组合的逐 shell L036/L040 上界之和为 `5`，因此 L046 在这个 checkpoint 上严格更强。
...
- L046：用精确大模数 hit-state union，跨 least-factor shells 汇总高带 cofactor-resource 占用。
...
下一步应直接攻击这三部分之一，或把 L046 与 mirror-incidence 约束连接起来；不应再为同一组二值 hits 引入另一套等价编码。
