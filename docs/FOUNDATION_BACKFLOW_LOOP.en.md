...
## 7. Initial regression examples

### FQ-20260809-004

The research answer has returned and is now in `STEWARD_VERIFICATION -> INTEGRATION`.

The smallest accepted scope is:

`typed state -> deterministic/observation functional kernel -> declared future-signature kernel`.

The whole P018 substrate is not promoted:
...
This question remains at `FQ_OPEN -> RESEARCH_SCHEDULED`: the stable `graph_distance` API has a broader operational domain than the P012 ordinary-metric theorem domain.

A5/P012/P022 geometry research must decide the API/domain layering. The foundation steward must not silently choose between narrowing the stable API and retaining a separately named directed shortest-walk helper.

These two examples intentionally occupy different parts of the loop so the full mechanism stays regression-testable.
