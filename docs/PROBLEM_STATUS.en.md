...
| P023 | `OPEN / ACTIVE RESEARCH` | `docs/P023_COMPOSITION_SAFE_COLLAPSE.en.md` and supplements: a coarse quotient is legal for a required future computation only when the relevant observables/operations descend to the quotient; current canonical-stage results cover fiber-constancy factorization, coarsest repair, finite operation-family closure, exact quotient/multiple-collapse compatibility, one-bit minimal repair, reductive gap-to-borrow transport, borrow telescoping, and stable safe-selector semigroup equivalence. The mother factorization/coarsest-repair statements are Lean-checked; general partition-refinement theory is prior art, while broader safe-precision structure remains active research. |
