...
| P003 | `RESOLVED` | `docs/P003_COLLAPSE_COMMUTATION.zh-CN.md`：完全幂坍缩算子全局交换，当且仅当正指数在整除序下可比；若 `p|q`，两个复合都等于 `C_q`；指数不可比时由 `2^max(p,q)` 给出显式素数幂见证。P003-T01 至 P003-T03 已在根构建实际 import 的 `EnterpriseMath/Arithmetic/CollapseCommutation.lean` 中全部通过 Lean 检查，包括完整反向与全局分类。 |
...
| P022 | `OPEN / ACTIVE RESEARCH` | Canonical executable geometry 现已包括 `lattice_geometry.py`；PR #262 的精确 `A_p`/simple-cubic geodesic multiplicity 与整数坐标 ABAB HCP contact-graph core；PR #288 的 periodic Barlow stacking core，包含精确 graph distance/geodesic multiplicity、FCC/HCP reconstruction 以及针对声明 root-to-target-layer query 的 cumulative interface-sign-count compression；PR #292 的 task-relative Barlow precision 层，其中 `(abs(k), delta_k)` 可精确恢复 effective interface counts 与选定层 vertical witness polynomial，而完整 prefix-imbalance trajectory 仍严格更丰富；以及 PR #296 的 periodic-growth 层，给出精确 drift-class growth equation `(lambda-2)^(2L)=2^(L+|D|)` 与 universal eventual C-finite recurrence class。相同 `(L,|D|)` 并不决定有限 shell-total sequence 或 phase。Coordination-observable 与 observation-history theory 仍不在此次 promotion 范围内；更广几何与跨 owner 接口继续活跃。 |
| P023 | `OPEN / ACTIVE RESEARCH` | `docs/P023_COMPOSITION_SAFE_COLLAPSE.zh-CN.md` 及补充：只有相关未来观察/运算能够通过粗商下沉时，该有限精度商才对所要求的未来计算合法。规范范围现包括 fiber 常值因子化、最粗修复、有限全操作族闭包、商/倍数坍缩精确兼容、one-bit 最小修复、reductive gap 到 borrow 的搬运、borrow 望远镜化、safe-selector semigroup 稳定等价、PR #249 的 Lean-checked 有界 quotient-root action-basis 特化，以及 FQ-006 对有限确定性**部分操作族**的合法性敏感扩展。对部分映射 `F_a:D_a->X`，safe quotient 必须同时保留 enabledness/定义域成员关系与目标 quotient class；有限细化给出最粗兼容细化，与有界动作词的合法性敏感 signature 一致，并在所有 `D_a=X` 时精确退化到全操作族构造。吸收态 `UNDEFINED` 只有在显式区分且 absorbing 时才可作为验证表示，不能被提升为 world-state 假设。见 `docs/P023_PARTIAL_OPERATION_QUOTIENT_SUPPLEMENT_08.zh-CN.md`、`src/enterprise_math/partial_operation_quotient.py` 及其回归测试。该 FQ-006 扩展为 `EXECUTABLE_CHECKED`，但未 Lean 检查。一般 partition refinement、automata distinguishability、partial-transition machinery、Test Cover 与 power-free arithmetic 均属于前人工作；更广义 safe-precision 结构继续活跃。 |
