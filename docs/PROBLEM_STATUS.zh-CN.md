...
| P022 | `OPEN / ACTIVE RESEARCH` | Canonical executable geometry 现已包括 `lattice_geometry.py`；PR #262 的精确 `A_p`/simple-cubic geodesic multiplicity 与整数坐标 ABAB HCP contact-graph core；PR #288 的 periodic Barlow stacking core，包含精确 graph distance/geodesic multiplicity、FCC/HCP reconstruction 以及针对声明 root-to-target-layer query 的 cumulative interface-sign-count compression；PR #292 的 task-relative Barlow precision 层，其中 `(abs(k), delta_k)` 可精确恢复 effective interface counts 与选定层 vertical witness polynomial，而完整 prefix-imbalance trajectory 仍严格更丰富；PR #296 的 periodic-growth 层，给出精确 drift-class growth equation `(lambda-2)^(2L)=2^(L+|D|)` 与 universal eventual C-finite recurrence class；以及 PR #298 的 coordination-observable 层，通过 prefix-imbalance quadratic energy 给出精确 shell/ball cardinality、精确 vertical-support cardinality `K(c,d)`、在 cardinality 本身不足时恢复 signed imbalance 的 first axial moments、对 extreme/cumulative drift-energy aggregates 的反演，以及周期 shell/ball 的首项系数。粗 shell/ball cardinality 不决定 signed drift allocation 或 geodesic-multiplicity structure。Observation-history 与 collision/repair theory 仍不在此次 promotion 范围内；更广几何与跨 owner 接口继续活跃。 |
