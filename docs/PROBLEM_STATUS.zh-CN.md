...
| P018 | `OPEN / ACTIVE RESEARCH` | 有限精度证明演算仍是持续扩展的基础研究计划。除已有 canonical 阶段外，PR #249 已把 P018↔P023 的有界 quotient-root future-action basis 正式集成并通过 Lean：对 `O_a(q)=R_r(floor(q/a))` 在 `0,...,N` 上，一个正 action set 分离全部精确状态，当且仅当它包含所有 `b<=N` 的正 `r`-power-free 整数；因此这些 actions 构成按包含关系唯一的最小 separating set。见 `EnterpriseMath/Quotient/RootAdjacentBoundary.lean`、`EnterpriseMath/Quotient/PowerFreeActionBasis.lean`、`src/enterprise_math/p018_p023_power_free_action_basis.py` 与 `docs/PRIOR_ART_P018_P023_POWER_FREE_ACTION_BASIS.zh-CN.md`。通用 Test Cover/distinguishing-family machinery 与 power-free arithmetic 属于前人工作；该精确 packaging 的历史新颖性仍未确认。整个编号计划继续保持活跃。 |
...
| P023 | `OPEN / ACTIVE RESEARCH` | `docs/P023_COMPOSITION_SAFE_COLLAPSE.zh-CN.md` 及补充：只有相关未来观察/运算能够通过粗商下沉时，该有限精度商才对所要求的未来计算合法；当前规范阶段覆盖 fiber 常值因子化、最粗修复、有限运算族闭包、商/倍数坍缩的精确兼容分类、one-bit 最小修复、reductive gap 到 borrow 的搬运、borrow 望远镜化、safe-selector semigroup 的稳定等价，以及 PR #249 的 canonical P018↔P023 有界 quotient-root action-basis 特化。后者已经 Lean 检查：在有界精确状态域中，分离全部状态所需且足够的 actions 恰是 `N` 以内所有正 `r`-power-free actions，因此得到唯一最小 separating action set。母层因子化/最粗修复已经 Lean 检查；一般 partition refinement/Test Cover 与 power-free arithmetic 属于前人工作，更广义 safe-precision 结构继续保持活跃研究。 |
