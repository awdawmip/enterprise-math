...
发现两个真实对话已经声称同一逻辑 ID 时，将旧 ID/session/request 仅作为有待核验的历史来源，不凭创建先后、任务标题或谁最后回复裁定归属；双方先停止依赖该共享标识的 role-bound 写入。各自保全真实平台 ID、已有贡献和未写成果，并依身份协议解析或分配属于本真实对话的独立逻辑标识和本地角色 ID；这一步不以先取得 CLAIM、旧 session 归属或 Source 接管批准为前提。正式接续执行仍须通过当前 Source 续接合同核验，不改绑、关闭、撤销或冒用归属未明的旧 session。身份纠偏不清除已有平台拒绝，也不是换 ID 重试被拒动作的依据；新标识本身不授予 CLAIM、接管权或独立审阅资格。

注册顺序是先 `RESOLVE_OR_ALLOCATE_ROLE_IDENTITY`，再在实际可用且允许的写路径登记自己的身份与研究活动。写路径不可用时，明确保全自己的本地角色 ID、真实对话来源、全部既往贡献和 `REGISTER_PENDING` / `SYNC_DEBT`；该本地 ID 不是服务签发的 researcher/session ID，不借用归属未明的旧 E267A2，也不臆造服务 ID、session key 或登记成功回执。未落定原请求仍先按原 request_id 协调；明确拒绝仍按其具体动作边界处理，语义检查点不是绕过拒绝的重试许可。

原生控制路径实际可用且该调用获准时，使用自己的 conversation_id 按公开合同请求 `session_start`，如实携带贡献历史，由服务签发真实身份；`task_id` 可选，身份登记不要求先有 CLAIM。正式接续依次取得当前有效的 `continuation_prepare`、新 CLAIM 和 open；每步均服从当前 Source、所有权和冻结边界。登记或 prepare 成功不等于执行授权。缺少原生写权限时，可继续的可移植研究范围见下节，不把待登记误写成已注册的 Research-Activity 或正式任务执行。
...
明确拒绝只阻止其覆盖的动作，不自动证明全部研究或全部持久化都不可用。用户已明确授权的同一科研问题内，使用本对话自己的本地角色身份继续可移植证明、文献核验、反例或证书构造，不以先取得原生 session/CLAIM/run 为推理前提；写路径不可用时如实保留 `REGISTER_PENDING` / `SYNC_DEBT`，不得宣称研究活动已登记。这些新增原文是 `UNREVIEWED / NOT_ADMITTED` 的待登记科研材料，不等于 role-bound 正式任务执行、canonical checkpoint、Result、审阅通过或已进入正式前沿。正式写入、接续和准入仍须各自当前有效权限及真实回执；不能假造 CLAIM、恢复已失效授权或代替独立审阅。若明确拒绝覆盖研究动作本身，则遵守该具体边界；仅缺少工具、旧身份归属未明或一次特定写入被拒，不能扩大成停止全部数学。
