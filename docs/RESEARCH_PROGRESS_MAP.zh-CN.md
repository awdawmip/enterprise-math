...
flowchart TB
  ...
  RB_SIX_BLOCK_REPAIR["RB 六块穷尽性修补"]
  JT2_QTF3_FIXED_POINT["JT2 固定点 p³ 纸面证明已检查"]
  R005_Q78553_SEAM["R005 扫描器已接受；精确目录开放"]
  A3 -->|"CONSUMES"| A4
  BRC -->|"CONSUMES"| A4
...
| **RB_SIX_BLOCK_REPAIR — RB 六块穷尽性修补**<br>RS-RB-SIX-BLOCK-COMPLETE-HALF-SECTION-OBSTRUCTION-REPAIR / TP2-8B673D783D3FBC81E124。固定 CM(-24) k/lambda 的 SOURCE_EXPOSED 证明修补。<br>[RB_OLD_BLIND_DRIVER_INTAKE](#source-rb_old_blind_driver_intake), [RB_OLD_BLIND_DRIVER_REVIEW](#source-rb_old_blind_driver_review), [RB_SIX_BLOCK_REPAIR_PUBLICATION](#source-rb_six_block_repair_publication), [RB_SIX_BLOCK_REPAIR_TASKBOOK](#source-rb_six_block_repair_taskbook) | 非作者辅助检查支持全部四个偶除子分区，包括 L(2O+T_r) 中被补偿的极点。U=R³+2Rt+R²−3 及 R·U=(R²+t)² 否定旧受限分类，但尚未证明或反驳完整六块对应定理。 | 尚无内核检查的证明。 | 驾驶员记录了精确整系数多项式辅助恒等式及篡改检查；该检查未解完整固定 k 系统，本索引未重跑数学。 | 任务书及匹配 V2 发布已在 main 1576703f。READY 定义与 dependencies=[] 表示已登记前沿，不等于 live CLAIM。 | DR-471BD65662D7933A46E8 停放旧未完成盲 Result；本快照中新修补任务自身尚无获接受 Result。 |
| **JT2_QTF3_FIXED_POINT — JT2 固定点 p³ 纸面证明已检查**<br>既有 QTF3 / TP2-C2C9DDEB2D65387D115E。以两项精确 Q2 输入为条件，对 p>3、p≡1 mod6 证明 L_p(1/2)=F_p(1) mod p³。<br>[JT2_CHECKED_QTF3_INTAKE](#source-jt2_checked_qtf3_intake), [JT2_QTF3_PEER_REVIEW](#source-jt2_qtf3_peer_review), [JT2_QTF3_PUBLICATION](#source-jt2_qtf3_publication), [JT2_QTF3_TASKBOOK](#source-jt2_qtf3_taskbook) | 系数逐项 p² 可除性及精确零前缀给出 D/p²=u^p r、deg r≤p−2。精确反射约束 deg(E−E∘ρ)≤p−1；模 p 后的高次系数强制 r+r∘ρ=0。在 u=1/2 处由单位2得到额外一个 p。这不是所有 u 的 p³ 多项式恒等式，也不是旧加权 LIFT。 | 仅纸面审阅，无 Lean/内核证书。 | 未使用数值推断或新数学程序；非作者报告检查精确 p−1 截断、合法除 p 及仅模 p 使用 Frobenius 的步骤。 | 证明接管及单独的同行报告已在 main 2358594e 包；未改变的第二代任务书允许以等价精确不变量代替微分路线。 | 有条件的非作者纸面 PASS，尚不是正式 Task Result 或 Driver 接受；已披露来源接触，作者登记时序保留实际记录。 |
| **R005_Q78553_SEAM — R005 扫描器已接受；精确目录开放**<br>R005 素数算法实验线；保留原 OBJ-R005-PRIME-ALGORITHM-LAB 与 relay OBJ-R005-PRIME-ALGORITHM-LAB-RELAY-20260909 的区别。既有 seam 任务 / TP2-09D6ECE7F315F0766FE1。<br>[R005_CURRENT_DRIVER_DOSSIER](#source-r005_current_driver_dossier), [R005_CORRECTION_DRIVER_REVIEW](#source-r005_correction_driver_review), [R005_CORRECTION_FOLLOWUP](#source-r005_correction_followup), [R005_SEAM_RELEASE_RECEIPT](#source-r005_seam_release_receipt), [R005_SEAM_PUBLICATION](#source-r005_seam_publication), [R005_SEAM_TASKBOOK](#source-r005_seam_taskbook) | 驾驶员保留 DSI1–3 及将 q=78553 归约为起点位于 [1291005053866735,1294364244470160] 的全部精确 916 连续素数间隙。认证前沿仍为 k≤2822453183433；可执行文件修复未取得目录完整性或 seam 定理。 | 软件/证据修复不声称新增定理或内核形式化。 | 非作者驾驶员记录最终字节编译、保留的定向回归、真实 attestation 的非空目录/扫描路径，以及覆盖范围与 attestation 拒绝检查。消费恢复出的原修复，未重复 DSI 或 seam 扫描。 | 合法双头归并保留两个发布与原 Result。正式审阅/followup 已入 main dbf8ee92，解锁/档案已入 main bd88c3ee。记录的 UNBLOCK5602494875 回放为 HANDOFF_READY/NEEDS_DISPATCH 且无 claim；执行时须刷新当前 runtime。 | DR-7605C20CFE02E3900E8D ACCEPTED 与 DFU-192EC00B45DD90358773 只关闭精确修复 Task，未新造任务或关闭父目标。此处 q78553 seam 尚无获接受 Result。 |
...
| 当前收尾 | RB_BLIND_BRANCH_PATTERNS | 已归档模式结果保留原未完成状态及有范围的接受。全局穷尽性需要完成新六块修补；原域下降及 1980 个条件参数族内部分类仍开放。 | 在已登记的完整半截面修补中消费保留的枚举及 RR/空纤维证明；保留单独获接受的精确映射，避免再次盲重做。 |
| 当前收尾 | RB_SOURCE_EXPOSED_EXACT_MAP | 单公式数学回传、Result、Driver 事务及 main 接管均完成。后续依赖修正保留此具体映射及全部 80 个输出 pin。1980 参数族只覆盖两个已研究分层；六块穷尽性、周期整数/同调指数及独立绝对归一化仍 OPEN。 | 历史盲交付接管现已完成并停放；由整线驾驶员继续既有源暴露六块修补，保留已接受映射，不重查未改变的数学。 |
| 当前收尾 | JT2_PERSISTENT_LINE | 固定点 p³ 纸面证明已检查，既有 QTF3 任务的正式交付仍待完成。Sun 第3项及条件第6项仍开放。旧加权 LIFT 现有显式进位/端口候选方程，尚未证明障碍为零。 | 使用真实 canonical 指派/CLAIM/ER，将已检查来源整理为 QTF3 六输出交付；另行检查新增双端口推论，再处理保留的 LIFT 标量及 Sun 义务。不需重复 Q2 证明或新造任务。 |
| 当前收尾 | JT2_QTF3_FIXED_POINT | 所述纸面范围内的证明义务已获支持；首个未完成交付单元是六输出正式回传及后续 Driver 记录。 | 通过真实任务执行绑定消费此已检查证明；区分 CM 端口地址1/2、固定点地址1、Sun 义务及加权 LIFT。 |
...
| 已有数学前沿 | RB_ENTERPRISE_THEOREM_PACKAGE_V2_INDEPENDENT_VALIDATION | 具体映射任务已获接受，整路线验证仍开放。历史盲研究接管已 PARKED，新登记的源暴露修补针对缺失的六块排除证明；1980 计数仅覆盖已研究的 4+2 与 2+2+2 分层。 | 由持续负责的 RB 驾驶员通过真实执行授权派发既有六块修补任务。周期整数、同调指数及独立绝对归一化仍是分开的父目标义务。 |
...
| 已有数学前沿 | RB_SIX_BLOCK_REPAIR | 须共同处理完整 L(3O)² 与 (R-r)L(2O+T_r)² 分区、算术常数及完整固定 k ODE；局部 pencil 或仅覆盖层解尚不足。 | 返回完整排除，或满足域/次数/纤维/未平方微分门槛的精确对应，或完整系统中最小的显式未解决边界。 |
| 已有数学前沿 | R005_Q78553_SEAM | 取得固定区间的完整、可独立审计的精确 916 目录；纪录间隙/首次出现间隙子集不能证明完整性。 | 真实研究员通过既有任务消费已接受扫描器，检查每个相关行/候选，返回整段认证、首个精确失败或不延伸前沿的显式完整性边界；在下一 q 前停止。 |
