...
flowchart TB
  ...
  RB_SOURCE_EXPOSED_EXACT_MAP["RB 已授权完整 ODE 检查点；任务 INCOMPLETE"]
  A3 -->|"CONSUMES"| A4
  BRC -->|"CONSUMES"| A4
...
| **RB_SOURCE_EXPOSED_EXACT_MAP — RB 已授权完整 ODE 检查点；任务 INCOMPLETE**<br>RS-RB-CM24-SOURCE-EXPOSED-EXACT-MAP-CERTIFICATE / TP2-554AC43234E8E297A6E9。独立发布的 CONTINUATION，只核一条固定恢复公式，明确 SOURCE_EXPOSED / NONBLIND_DISCLOSED；不重开盲分类，不覆盖全部存活族。<br>[S113](#source-s113), [S114](#source-s114), [S115](#source-s115), [S116](#source-s116), [S117](#source-s117), [S119](#source-s119), [S120](#source-s120), [S121](#source-s121), [S122](#source-s122), [S123](#source-s123), [S124](#source-s124), [S125](#source-s125), [S126](#source-s126) | 已执行的 identity_v1 整数多项式检查对完整清分母冻结 ODE 给出空余式，未代入临界除子：FULL_FUNCTION_FIELD_ODE_CLEARED_IDENTITY_PASS。所写有理 Y 给出有条件的目标方程推论，仍需非零/嵌入证明。这是一个恒等式检查点，不是六次映射证书、未平方微分证明或全任务完成。 | 不声称 Lean 证明或形式化精确映射结果。 | 已发布的 canonical authorize 回执在 2026-09-09 00:32:37 UTC 为 comment 5593861263 与 ER-BDAB13EB167775B80BE7 记录 CURRENT_AUTHORIZED_WINNING_ISSUE_240_CLAIM。随后实际 identity_v1 运行于 00:43:07 UTC 以 exit 0 结束，SHA256 绑定该授权与证书。程序记录整数系数运算及代数关系重写；BRC 求值、整数除法、根式求值均为零。T0/T6 lookup 仍是合同层复用，不是原生数学执行声明。本图只引用既有运行回执，不授予当前 lease 权限。 | 任务书与 TP2-554AC43234E8E297A6E9 保持已发布于主线 3a00ea191ad0380bf341af746a825e0b0c474874；活动 RA-81196A3A34AE5695CB527CFF 与 ER 也已分别持久化至主线。四份 identity_v1 检查点、证书、运行与授权来源固定在 source 提交 341f36bb53c25d97c3ae533a71c92ad273a7b904。本检查点的 PR1426 为 draft，main intake 待办；研究分支持久化不等于主线接受。 | 前盲阶段保留 TASK 范围的 ACCEPTED/ARCHIVE 判词。新精确映射检查点明确仍为 INCOMPLETE，尚无正式 Result 或 Driver 接受。局部恒等式 PASS 不授予全任务 PASS、Working Truth、API、Foundation promotion 或父目标闭合；源暴露不是盲验证。 |
...
| 已有数学前沿 | RB_SOURCE_EXPOSED_EXACT_MAP | RB_CM24_PINNED_SOURCE_EXACT_MAP_CERTIFIED_OR_EXACT_FAILURE 仍 OPEN / INCOMPLETE。仍须完成：域/嵌入非零证书；全部特殊纤维赋值及 square-class 定位/下降；全部共同基点消去，以及 descended X 与 D 到 E 两项六次度数证明；精确目标 j/模型关系；含 Y/符号/嵌入与异常点正则性的未平方微分；独立反例检查及完整证明/回传。周期整数/同调指数、独立绝对周期比、全部 1980 参数族分类仍是另列的 OPEN 父缺口。 | 在有效的当前执行绑定下，从冻结 identity_v1 前沿继续既有任务，保留来源与全部失败。完成剩余精确关卡或给出指名障碍，纳入未平方符号与篡改检查；任何关卡未解均保持 INCOMPLETE。主线收录、正式审查与已记录的恒等式运行分别处理，不重开盲分类，不推出参数族或周期闭合。 |
