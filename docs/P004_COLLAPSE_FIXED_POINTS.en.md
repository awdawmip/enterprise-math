...
This complements the canonical P009 typed collapse+coarsening result: typed trajectories terminate under the stated strict transitions, while mixed collapse/projection schedules need not be confluent. P004 isolates a separate order-insensitive invariant of the collapse-only subfamily: its fixed-state set.
...
## 8. Consequence for collapse-word dynamics

P004 supplies the fixed-state part of the collapse-only dynamics problem.

For a fixed collapse word \(W\), repeated iteration produces a nonincreasing integer sequence because \(W(n)\le n\). Hence every orbit stabilizes after finitely many strict decreases. Once stabilized, P004-T03 forces the limit to be a perfect \(L\)-th power.

A stronger follow-up is available: the limit can be sharpened from “some perfect \(L\)-th power” to the exact candidate

\[
C_L(n_0).
\]

That stronger statement is **not** silently added to the already-resolved P009 scope here. If promoted canonically, it should be tracked as a separate numbered follow-up or an explicitly scoped supplement.
