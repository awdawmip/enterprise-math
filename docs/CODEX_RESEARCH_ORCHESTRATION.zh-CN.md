...
Owner 负责路线组合与控制面，路线驾驶员负责一条线的研究闭环，研究员负责一个有界任务。线路责任和审阅台账跨执行会话持久保存；普通成果不逐件返回 Owner 重审。Owner 与路线驾驶员都是现有 `RESEARCH_DRIVER` 的职责范围，不是新的身份枚举。
...
“常驻”指线路、任务和证据持续存在。原代理仍可用时可以继续；原代理消失后，任意有相应授权的新对话都应通过规范续接入口接手，不以联系原人为前提。从已验证前沿和作者／曝光台账恢复，不重置 Task-ID 或数学进度；旧执行权通过明确、认证的所有权或权限转换被隔离。见[跨对话续接协议](RESEARCH_CONTINUATION_PROTOCOL.md)。
