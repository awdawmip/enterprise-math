...
An empty dependency list uses the normal assigned entry. A nonempty list is supported only for the explicit artifact-obligation model described below. Unsupported, malformed or unresolved dependencies return scoped `NO_DISPATCH`, `selection_status: BLOCKED`, and a blocked target. The result concerns the requested task only; it does not assert that the global queue is empty. Do not remove dependencies or narrow the task to make the entry accept it. Canonical quarantine, parent closure/parking, hard blocks, pending Result/review state and cohort ownership remain enforced by the existing reducer.
...
### Artifact dependencies and typed release evidence

Each supported dependency has exactly `task_id` and a nonempty `required_artifact` description, with no duplicate task IDs. The requirement text is not parsed as a Boolean or an acceptance label. An actual Driver's typed, authenticated release expresses its assessment of the declared obligation; the checker verifies the exact current bindings and operational evidence. It does not perform mathematical review or mark the obligation task DONE.

The source must be a canonically applied, unedited Issue 240 `UNBLOCK` bound to the target's current TP2, issued under the Driver authority active at that server time. The latest applied release governs this evidence; a later applied hard block or terminal transition invalidates it, and a later untyped release cannot silently reuse the older proof. Revoking or replacing a Driver's authority later does not rewrite a valid historical release. Current assignment authority remains independently required.

For one obligation, the existing typed release fields are consumed directly: `review_obligation_task_id`, `review_obligation_publication_id`, `gate_evidence` and the exact review URL in `progress_ref`. For several obligations, use a `dependency_gate_evidence` list and omit those singular fields. Every list item has exactly `task_id`, `publication_id`, `gate_evidence` and `review_ref`, and the list must cover every declared obligation exactly once.

Each `gate_evidence` object requires `canonical_main`, `result_id`, `result_record_sha256`, `review_id` and `followup_id`; `accepted_strength` is optional descriptive provenance. `review_ref` (or the singular `progress_ref`) must be the exact immutable GitHub URL for that review at `canonical_main`. The historical link is not used as a substitute for current evidence: the obligation's TP2/taskbook and the underlying Result publication must still be operational, the current raw Result bytes must match the released digest, the exact immutable DR must still be the current ACCEPTED review, and its exact DFU must be ready and bound to the same Result digest. Review and follow-up timestamps must precede the release. Parallel/synthetic review authorities and other dependency languages need their own canonical model; this entry does not infer them from names or prose.

The full proof is returned as `dependency_release_evidence`. Compact startup retains a fixed-size summary in `assigned_research_selection.dependency_release`: release comment ID, body digest, obligation count and the SHA256 of the full proof serialized with sorted keys, compact separators and UTF-8. The normal current-task, owner/session, prepare, CLAIM and authorize gates still follow. No dependency, original record, claim or parent state is rewritten by this read-only verification.
