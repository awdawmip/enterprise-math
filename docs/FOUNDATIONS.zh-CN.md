...
进一步地，P024 的 material-observable 特化已经给出一个严格的“任务不可见状态删除”实例：即使几何层能够区分不同 raw depth，只要它们在声明 horizon 内产生完全相同的 material response future word，那么这些 raw depths 在材料任务的最粗精确 quotient 中就应合并。该结果甚至不要求响应值单调；真正决定精度的是完整未来响应词，而不是未经声明的内部几何细节。

特别地，即使名义测量尺度没有变化，动作语言、观测语言与状态拓扑也可以改变最小未来安全精度。这首先是一条关于**预测充分性**的数学结论，并不自动等于“自然的物理本体会随任务任意变化”的物理主张。
...
但保留 exact window 再计算真实 root image，L055 证明实际 lower-band shell images 从
