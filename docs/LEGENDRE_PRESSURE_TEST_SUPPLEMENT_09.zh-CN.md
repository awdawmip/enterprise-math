...
## 3. L040 —— Anchor-surviving 的光滑闭合判据
...
知 \(q\) 是小素数。又因为命中通过 anchor 筛选，故 \(q\nmid A_k\)，因此 \(q\nmid M\)。于是 \(q\) 必为横向小素数。
...
anchor 素数都整除 272，因此该命中通过 anchor 筛选。其 cofactor 为
...
是假的。L040 中 anchor-survival 条件是逻辑上必须的，不是表述偏好。

---

## 6. 与当前 P017 主线的关系

L040 不建立第二套大模数路线。它只细化规范 L039 命中 bit 触发之后的结构：
