...
The analytic step after the exact high-band reductions uses classical prime-distribution estimates, not new project mathematics. Rosser and Schoenfeld give standard explicit estimates for prime-counting functions and for the reciprocal-prime sum, in the classical Mertens neighborhood. [SRC-ROSSER-SCHOENFELD-1962-PRIME-ESTIMATES]
...
and that the number of primes up to `x` is `o(x)` (indeed much more precise estimates are classical). The project uses these only after it has already produced exact finite prime intervals and exact finite hit-count envelopes.
...
The later high-band route then builds exact pairwise resource separation, multiplicative capacity, and cross-shell hit-state unions before invoking any analytic prime-distribution theorem. L050 uses the classical reciprocal-prime estimate only after the project has produced two square-derived finite intervals:

\[
\sqrt{2k}\le p\le k
\]

for the least primes of **all** high-band composite shells, and the narrower

\[
\sqrt{2k}\le r\le\frac{k+2}{2}
\]

for the cofactor resources in the L049 three-prime union. Classical Mertens behavior on these project-derived endpoints yields the `log(2)` all-high-band composite ceiling and the `log(2)/2` three-prime fraction ceiling. Neither constant is claimed as a new prime-distribution law.
