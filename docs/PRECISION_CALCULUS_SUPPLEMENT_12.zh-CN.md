...
## 4. P018-T110 — 二盆地 quotient transport
...
T110 说明其中每个元素的 square-root index 都只能是 `j` 或 `j+1`。
...
T110 改变了 P017 中 lower-band 问题的定位。
...
因此下一步 P017 应优先检验通过 T110 对 lower band 做递归运输，而不是再引入一套平坦筛法编码。真正的问题变成：反复提取因子并配合二盆地 transport 后，能否形成足够强的良基有限下降，从而控制剩余 lower-band composite mass。
