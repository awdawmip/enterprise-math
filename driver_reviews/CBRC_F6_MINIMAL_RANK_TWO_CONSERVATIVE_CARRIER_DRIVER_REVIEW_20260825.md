...
- embedding only;
- old-signed retraction `pi:C->Z e`;
- full upstream retract `r:C->C1`.

A full additive retract exists exactly when the distinguished order-three class satisfies `tau notin 3T`; equivalently there is `phi:T->Z/3` with `phi(tau)=1`. Thus a larger pointed torsion carrier such as `T=Z/9, tau=3g` can preserve the upstream embedding and old projection while failing to admit a full upstream retract.

At the least carrier `T=Z/3`, a full retract exists automatically, but choosing one is extra data and is not required for the least class.
...
These yield exactly `22` raw lifts.

Under typed complement gauge, the `22` solutions form exactly `6` equivalence classes. Driver independently recomputed the finite parameter set and orbit count and obtained the same `22 / 6` classification.

Modulo torsion:

- `R` fixes the new free direction;
- `J` and `S` can only act by sign.

Therefore no genuine order-three unary orbit on the new free quotient is derived at F6.
...
The embedded relations remain exact, including

`e+J e=0`,

`e+J R e=-tau != 0`.

No new relation collapses `e` or `tau`. No multiplication is introduced.
...
The completeness claim is accepted from the theorem derivation; bounded enumeration is regression evidence only.

## 8. Scope narrowing

F6 does **not** derive:

- a multiplication on `C_min`;
- a ring/field/module beyond the additive group already used;
- a norm, inner product, quadratic form, square law, or probability law;
- a complex/quadratic-integer interpretation;
- a two-slot mixing operator;
- a unique scalar law;
- a wave or continuum structure.
