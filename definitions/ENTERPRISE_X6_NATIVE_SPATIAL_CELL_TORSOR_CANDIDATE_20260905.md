...
Define the pure coordinate adjacency relation by the 12 actions `T_i^{+/-1}`.

They are 12 distinct directed coordinate neighbours at every spatial Cell.

Freeze:

`X6_PURE_COORDINATE_ADJACENCY = SIX_POSITIVE_AXIS_STEPS + THEIR_PATH_REVERSALS`.

`PATH_REVERSAL != NEW_NATIVE_NEGATIVE_AXIS`.

Additional carrier/channel adjacency, if later declared, must be separately typed rather than silently changing this coordinate graph.
...
A unit positive-axis displacement has squared gauge 1; its reverse decoder has five 1s and one 0, squared gauge 5. Projection to any selected three-axis slice containing that axis recovers the established local reverse decoder `(0,1,1)` and squared gauge 2.

## 7. Native 3-axis slice observations

For every 3-subset `S` of the six axes define

`Obs_S([z])=can3(z|_S)`.

There are 20 native three-axis coordinate selections.
...
Therefore omitted axes act on the full spatial Cell while being exactly invisible in that selected coordinate observation.
...
This is the exact spatial meaning of P000 omitted-coordinate semantics.

## 8. Slice loops versus full spatial state

For `S={i,j,k}`, define

`H_S=e_i+e_j+e_k in G6_D`.

The selected slice observes

`Obs_S(H_S)=0`,

so the familiar positive three-axis triangle closes in the slice coordinate/Cell observation.
...
Thus a lower-dimensional closed slice trajectory may carry omitted-coordinate spatial holonomy.

Promoting every 3-axis triangle to a full X6 return would collapse all six axis actions to `C3`, so such promotion is forbidden absent a narrower explicit relation.

## 9. Rotation skeleton

Every permutation of the six native axes preserves the coordinate torsor structure, min-zero section, pure coordinate adjacency, pairwise Enterprise orthogonality and sum-of-squares component gauge.

Freeze the finite positive-axis permutation skeleton

`X6_AXIS_PERMUTATION_ROTATION_SKELETON = S6`.

This is not a claim that every native rotation is only an axis permutation.

The existing FCC/K4 atlas-preserving group embeds as

`S4 < S6`.

Under that carrier subgroup the 20 native 3-axis selections split into 4 K4 stars, 4 K4 faces and 12 K4 path triples.

The local visible C3 is the `A3` subgroup of the visible S3 stabilizer of any selected three-axis slice.
...
may be used as models of **additional Cell decoration/internal relational state** over X6. The common-depth cocycle tool classifies these covers exactly.

They are not alternative six-dimensional spatial coordinate spaces under this definition.
...
For pure positive six-axis coordinate paths of length m with min-zero endpoint a, exact terminal N-BRC multiplicity is
...
The canonical endpoint grade is `sum a_i mod6`.

Path-formal BRC remains above endpoint coordinates and retains ordering/provenance/multiplicity as required by the observer.

## 12. Admission boundary

This candidate is ready for Foundation admission only if the project accepts the P000-derived typing statement:

`FULL NATIVE SPATIAL CELL CENTER IDENTITY IS THE SIX-AXIS COORDINATE TORSOR`.

It does not claim every dynamic/internal state of a Cell is exhausted by coordinates, does not make the geometric origin a Cell, and does not identify FCC carrier Euclidean coordinates with native six-dimensional coordinates.
