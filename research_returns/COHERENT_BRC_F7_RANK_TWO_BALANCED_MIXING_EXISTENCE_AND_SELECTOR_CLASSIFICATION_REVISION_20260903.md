...
`Per(f) op Per(f)`: if `(u,v)` is a pair period, then
...
Call the rational line `L=Q ell` a **hub line**.

`J`-invariance transports hubs to hubs. If `L` and `JL` are distinct, they are
independent in `Q^2`; on the finite-index lattice generated by the two hub
