...
def numeratorShift : Fin 5 → ℚ := ![1, 3 / 4, 1 / 2, 1 / 4, 0]
def denominatorShift : Fin 5 → ℚ := ![5 / 6, 4 / 6, 3 / 6, 2 / 6, 1 / 6]
def numeratorPrefix : Fin 6 → ℚ := ![0, 1, 7 / 4, 9 / 4, 5 / 2, 5 / 2]
def denominatorPrefix : Fin 6 → ℚ := ![0, 5 / 6, 3 / 2, 2, 7 / 3, 5 / 2]
def prefixDifference : Fin 6 → ℚ := ![0, 1 / 6, 1 / 4, 1 / 4, 1 / 6, 0]
...
theorem prefix_difference_exact (r : Fin 6) :
  ...
  fin_cases r <;> norm_num [numeratorPrefix, denominatorPrefix, prefixDifference]
