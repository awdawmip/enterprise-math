...
theorem finite_tail_le_geometric_sum
    ...
    (hratio : ∀ n, term (n + 1) ≤ q * term n)
    (M N : ℕ) :
    (∑ j ∈ Finset.range N, term (M + 1 + j)) ≤
      (∑ j ∈ Finset.range N, q ^ j) * term (M + 1) := by
  calc
    (∑ j ∈ Finset.range N, term (M + 1 + j)) ≤
        ∑ j ∈ Finset.range N, q ^ j * term (M + 1) := by
      exact Finset.sum_le_sum fun j _ =>
        term_le_geometric term q hq hratio M j
    _ = (∑ j ∈ Finset.range N, q ^ j) * term (M + 1) := by
      rw [Finset.sum_mul]
...
theorem geom_sum_mul_one_sub (q : ℝ) (N : ℕ) :
  ...
  | succ N ih =>
      ...
      calc
        ((∑ j ∈ Finset.range N, q ^ j) + q ^ N) * (1 - q) =
            (∑ j ∈ Finset.range N, q ^ j) * (1 - q) + q ^ N * (1 - q) := by
          ring
