...
theorem edgeSum_pairAverage (x : QEdge) : edgeSum (pairAverage x) = edgeSum x := by
  simp [edgeSum, pairAverage]
  ring

theorem antiResidual_zero_sum (x : QEdge) : edgeSum (antiResidual x) = 0 := by
...
theorem kernelPotential_zero_sum (r : QEdge) : vertexSum (kernelPotential r) = 0 := by
...
theorem delta_kernelPotential_of_antisymmetric
  ...
  funext e
  fin_cases e <;> simp [delta, kernelPotential] <;> linarith

/-- Canonical rational splitting into slice-induced and opposite-pair-average parts. -/
theorem rational_splitting (x : QEdge) :
  ...
  have hdelta : delta (kernelPotential (antiResidual x)) = antiResidual x :=
...
theorem pairAverage_idempotent (x : QEdge) : pairAverage (pairAverage x) = pairAverage x := by
  funext e
  fin_cases e <;> simp [pairAverage] <;> ring

theorem antiResidual_pairAverage (x : QEdge) : antiResidual (pairAverage x) = 0 := by
  funext e
  simp [antiResidual, pairAverage_idempotent]

theorem pairAverage_antiResidual (x : QEdge) : pairAverage (antiResidual x) = 0 := by
  funext e
