...
theorem reciprocalApproximation_strict_step
    ...
    reciprocalApproximation A term (M + 1) < reciprocalApproximation A term M := by
...
theorem reciprocalApproximation_above_exact
  ...
  unfold reciprocalApproximation
  exact one_div_strictAnti (partialSum_pos A term M hA hterm_nonneg) hbelow

theorem reciprocal_error_identity {s t : ℝ} (hs : s ≠ 0) (ht : t ≠ 0) :
    1 / s - 1 / t = (t - s) / (s * t) := by
  field_simp [hs, ht]
  ring
