...
/-- A quotient observation is compatible with one `k`-ary operation exactly when
coordinatewise observational equality forces observational equality of outputs.

Semantic replay from historical P018 `OperationCongruence`; this is the A2
owner form and adds no new mathematical claim. -/
...
exact descended operation on the quotient state space. -/
...
/-- Exact finite-arity descent is equivalent to congruence of the observation
kernel when the observation is surjective. -/
...
/-- Binary compatibility written without `Fin 2`. -/
def BinaryObservationCompatible
    {α β : Type*} (O : α → β) (μ : α → α → α) : Prop :=
  ∀ a a' b b' : α,
    O a = O a' → O b = O b' → O (μ a b) = O (μ a' b')

/-- Compatibility when only the left input is replaced in its observation fiber. -/
def LeftObservationCompatible
    {α β : Type*} (O : α → β) (μ : α → α → α) : Prop :=
  ∀ a a' b : α, O a = O a' → O (μ a b) = O (μ a' b)

/-- Compatibility when only the right input is replaced in its observation fiber. -/
def RightObservationCompatible
    {α β : Type*} (O : α → β) (μ : α → α → α) : Prop :=
  ∀ a b b' : α, O b = O b' → O (μ a b) = O (μ a b')

/-- Full binary compatibility is equivalent to the two elementary
one-coordinate replacement tests. -/
