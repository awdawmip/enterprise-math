...
/-- P018-T119: composition of three successive endpoints forgets only the
intermediate point and returns the outer pair. -/
@[simp] theorem composeAdjacent_mk {α : Type} (a b c : α) :
    composeAdjacent (a, b) (b, c) rfl = (a, c) := rfl

/-- P018-T120: the two decompositions of a deterministic 2x2 rectangle have
exactly the same outer endpoint pair.  No additive, order, metric, or topological
