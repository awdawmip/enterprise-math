...
/-- The basic parity generator is genuinely nonzero modulo the zero-sum delta image. -/
theorem basic_generator_nontrivial :
    ¬ ∃ v : VertexData,
      vertexSum v = 0 ∧ delta v = edgePattern 1 0 0 :=
  basic_parity_class_not_mem

/-- Twice the basic generator is represented by one and only one zero-sum vertex potential. -/
...
/-- The two certificates together express exact order two at the image-obstruction level. -/
theorem basic_generator_exact_order_two_certificate :
    (¬ ∃ v : VertexData,
      vertexSum v = 0 ∧ delta v = edgePattern 1 0 0) ∧
    (∃! v : VertexData,
      vertexSum v = 0 ∧ delta v = edgePattern 2 0 0) := by
  exact ⟨basic_generator_nontrivial, twice_basic_generator_unique⟩
